...
This document describes method of how to address customization of different
...
approved design. Some or even all assumptions here might changed depending on
...
DBus calls to appropriate DBus Service. DBus Service shall implement certain
standardize signals, as well as specially defined Redfish DBus interfaces.
...
While lookup succeed it will make a direct DBus call to appropriate method
(HTTP method mapped 1:1 to DBus methods) and will retrieve a DBus object
...
The signal handler shall search for newly create/removed DBus interfaces:
...
## OEM Application exposing Redfish DBus interfaces

The OEM application that would like to serve Redfish over DBus, shall implement
...
This interface shall be implemented only on items, that are part of collection,
and shall consists of one property only `CollectionId` which describes collection Id
it will link to.
...
### Python MOCK example:
Below is a fully functional example of implementing simple OemExtention in Python.
