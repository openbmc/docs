...
groups (i.e. User will have same privilege for REDFISH / IPMI / SSH / HostConsole).
User can belong to any one of the following privilege roles at any point of time.
...
5. If there is no mapping for group name to privilege role,
   the session creation is rejected. The LDAP group to privilege role mapping
   must be configured for the user to be able to authenticate.
