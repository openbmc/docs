...
Several BMC services to the BMC (via devices such as /dev/ttyS0, referred to as
HOST_TTY) which connects to the host serial port.
...
default configuration (which serves the HTTP application protocol over the
...
intent is to help you understand the relevance of each item locate details in
the source code.
...
Connection to a network is optional.  An alternative configuration has the host
system providing the BMC's network, optionally with no external management
connection (indicated by the letter X), as shown in this diagram:
```
            +-----------+      +----------------+
            | BMC       |      | Host           |
management  |           |      |                |
network X-+-+- Network  |      |       Network -+- payload
          | |           |      |                |  network
          | |           |   +--+- management    |
          | |           |   |  |  network       |
          | +-----------+   |  +----------------+
...
Contrast with the HTTP application protocol which is provided by the web
server.
...
Refers to the host system's console access, typically provided via the host
serial access.  Contrast with the BMC's serial console access.  See also
`obmc-console-server` which provides access to the host console internally to
BMC services.
...
detailt provided by [IPMI Session management][].  network IPMI provides access
...
Refers to the command used by agents on the management network to reach the
BMC's network IPMI service.  Hosted by https://github.com/ipmitool/ipmitool
...
BMC's intertnal and external interfaces.

### hostlogger

Refers to the BMC service provided by the `hostloggger` program here:
...
configurations options to:
 ...
 - enable the HTTP transpot protocol instead of HTTPS
...
details here: https://github.com/openbmc/bmcweb/blob/master/Redfish.md And docs
...
(KVM).  See https://github.com/openbmc/obmc-ikvm/blob/master/README.md
