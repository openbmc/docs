...
associationPath. If no interfaces are passed in, then
all objects/services/interfaces in the subtree and associated entpoint
are returned. If interfaces are passed in, then only those interfaces
...
- Map of object paths to a map of service names to their interfaces that
  is in the associated endpoints
