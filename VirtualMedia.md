...
and and operates in RO mode, or RW mode (keeping in mind container limitations,
...
ISO/IMG image mounting, and remotely CIFS/HTTPS image mounting.
...
The component diagram below shows Virtual Media high overview
...
Proxy Mode uses browser Java Script and WebSockets support, to create JS NBD
...
encryption mechanisms directly from HTTPs server, all data transactions goes
through bmcweb.
...
Legacy Mode uses defined by DMTF schemas for Virtual Media for mounting
external CIFS/HTTPs images. The current implementation supports only stream
mounting as for now. In this case Redfish is only used as mechanism for Virtual
Media initialization, and is not responsible for data transmission. For data
there is a separate component responsible for handling CIFS/HTTPs traffic
called NBDkit.
...
mechanism for caching image, either kernel has home buffer mechanisms for block
...
Virtual Media is separate application that will coexist on DBus. It will be
initialized from configuration json file, and expose all available for
