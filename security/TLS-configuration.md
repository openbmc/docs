...
`keyUsage`, `extendedKeyUsage`, `authorityKeyIdentifier` attributes using your
favorite text editor. It should contain the certificate extensions shown below.
```
[ my_ext_section ]
keyUsage = digitalSignature, keyAgreement
extendedKeyUsage = clientAuth
...
```
openssl req -new -config openssl.cnf -key CA-key.pem -x509 -days 1000 -out CA-cert.pem
```
...
{
  "CertificateString":"$(cat CA-cert.pem | sed -e ':a;N;$!ba;s/\r\?\n/\\n/g')",
  "CertificateType": "PEM"
}
END
```
...
```
curl --user root:0penBmc -d @message_body -k -X POST https://${bmc}/redfish/v1/Managers/bmc/Truststore/Certificates
