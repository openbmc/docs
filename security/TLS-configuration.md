...
## Certificates
For certificate to be marked as valid, it (and every certificate in chain)
...
Find the sections listed below and add or change presented values.
...
Now create a server configuration `openssl-server.cnf` by copping the client
file
...
Create two additional configuration files `myext-client.cnf` for client and
`myext-server.cnf` for server certificate. Without these files no extensions
...
```
You might be prompted for passphrase that must be entered each time the key
is used, thereby protecting key files from unauthorized use. A passphrase
is a word or phrase that protects private key files. It prevents unauthorized
users from encrypting them.

Now we can create CA certificate using previously generated key. You will
...
in this case as a **CommonName** provide OpenBMC system user name (for
example **root**).
...
information as during CA generation, but in this case as a **CommonName**
provide the fully qualified domain name of your OpenBMC host. In this example
it will be `bmc.example.com`. A wildcard can be used to protect multiple
host, for example a certificate configured for `*.example.com` will secure
www.example.com, as well as mail.example.com, blog.example.com, and others.
...
Where ${bmc} should be `bmc.example.com`. It is convenient to export it as
...
OpenBMC by default. If we want to use the certificate signed by our CA it
must be replaced. Additionally we must upload to OpenBMC the private key
that was used to sign the server certificate. A proper message mody can be
prepared the with this command:
