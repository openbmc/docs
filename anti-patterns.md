...
means with a with a shift in system design philosophy. As such, GPIOs are a
(hardware) implementation detail. Exposing the implementation on DBus forces the
...
and therefore hard to abstract. That said, if this is the case, then the
functionality in question is probably best wrapped up as an implementation
detail of a behaviour that is more generally applicable (e.g. host power-on
procedures).
