...
SSH).  User can belong to any one of the following privilege role at any point
of time.
...
|Sl. No| Method /<br> Property /<br> Signal | Parameters | Return Codes / Error | Comments |
|-----:|--------------------------|-------------------------|------------|----------------------|----------|
|1     |CreateUser <Method>          | **UserName** - STRING <br> **GroupName** - STRING[] - List of groups to which the user has to be created <br> **Privilege** - STRING - Privilege of the user to be added. <br> **Enabled** - BOOL - true / false to indicate enabled / disabled state. Note: Implementation can elect to accept NULL (empty) Privilege. | success<br>UserNameExists<br>UserNamePrivFail<br>NoResource<br>InvalidArgument<br>InternalFailure<br>InsufficientPermission|1. UserName has to be unique (if UserName already exists, then UserNameExists will be thrown)<br>2. If 'ipmi' Group role is chosen, then UserName should not exceed 16 bytes else UserNamePrivFail will be thrown |
|2     |RenameUser <Method>       | **UserName** - STRING <br> **NewUserName** - STRING | sucess<br>UserNameDoesNotExist<br>UserNameExists<br>InvalidArgument<br>InternalFailure<br>InsufficientPermission||
