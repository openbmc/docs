...
user management (phosphor-user-manager), where-as individual user related
settings for any application has to be managed by that application. I.e. other
than, User name, Password, Group and Privileged role, rest of the settings
needed, has to be owned by that application. (e.g. IPMI daemon has to manage
setting's like channel based restriction etc. for the corresponding user).
Design is made to cover this scenario.
...
User can belong to any one of the following privilege role at any point of time.
...
|3     | user | Users are allowed to have only read only access and can't change any behavior of the system.|
|4     | callback   | Lowest privilege level, only allowed to initiate IPMI 2.0 specific callbacks.|
|5     | custom     | custom privilege level. This is only allowed for REDFISH only user group.|
...
|Sl. No| Method /<br> Property /<br> Signal | Privilege <br>Required  | Parameters | Return Codes / Error | Comments |
|-----:|--------------------------|-------------------------|------------|----------------------|----------|
|1     |AddUser <Method>          | USER-MANAGER            | **UserName** - STRING <br> **GroupName** - STRING[] - List of groups to which the user has to be created <br> **Privilege** - STRING - Privilege of the user to be added. <br> Note: Implementation can elect to accept NULL (empty) Privilege. This is to create disabled user account and to support IPMI 'SetUserName' as first command, before 'SetUserAccess'| success<br>UserNameExists<br>UserNamePrivFail<br>NoResource<br>InvalidArgument<br>InternalFailure<br>InsufficientPermission|1. UserName has to be unique (if UserName already exists, then UserNameExists will be thrown)<br>2. If 'ipmi' Group role is chosen, then UserName should not exceed 16 bytes else UserNamePrivFail will be thrown |
|2     |DeleteUser <Method>       | USER-MANAGER            | **UserName** - STRING | success<br>UserNameDoesNotExist<br>NoResource<br>InvalidArgument<br>InternalFailure<br>InsufficientPermission||
|3     |ListGroups <Property>     | USER-MANAGER            |             | **Groups** - STRING[] - Lists all supported Groups in the system. Only Groups in this list will be accepted for AddUser method | |
|3     |ListPrivileges <Property> | USER-MANAGER            |             | **Privileges** - STRING[] - List all supported privileges in the system. One privilege in this list or NULL(empty) will be accepted for AddUser method | |
|4     |UserUpdate <Signal>       |                         | **UserName** <br> **GroupName** <br> **Privilege** <br> **ChangeValue** - Created/Deleted/PrivilegeUpdated/GroupUpdated||||
...
|Sl. No| Method /<br> Property /<br> Signal | Privilege <br>Required  | Parameters | Return Codes / Error | Comments |
|-----:|--------------------------|-------------------------|------------|----------------------|----------|
|1     |UserPrivilege <Property>  | USER-MANAGER            |            | **Privilege** - STRING - User privilege. ||
|2     |UserGroups <Property>     | USER-MANAGER            |            | **GroupNames** - STRING[] - Groups to which user belongs. |||
...
```
    ...
    ||======================|======||                   |    ||                        ||
                            V                           |    || |********************| ||
                            |                           ^    || | IPMI specific 1:1  | ||
                            | UserUpdate(Signal)        |    || | user mappings      | ||
                            +------>-------------->-----+------>| storage            | ||
                                                             || | Note: Either Host  | ||
                                                             || | / Net IPMID must   | ||
                                                             || | implement signal   | ||
...
getting added to the 'ipmi' Group. Phosphor-user-manager has to verify this
limitation.
