...
Therefore, an alternative solution is directly providing both a universal health
interface, `xyz.openbmc_project.State.Decorator.Health`, and an assocation of
descendants, `health_rollup`, on dbus.
`xyz.openbmc_project.State.Decorator.Health` has a `Health` property with three
possible values which are just "OK", "Warning" and "Critical". `health_rollup`
includes all descendant object paths but not just those of direct
children. The rollup health can be easily figured out by checking all `Health`
properties of the object and its descendants.
