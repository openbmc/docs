...
information. This type of dump can be generated by user. This dump gets
collected on BMC and stored on on BMC

#### System Dump
A system dump is a collection on debug information from the host, this includes
host memory and register data. This dump is initiated by BMC and there will be
system reboot while collecting the dump. Dump gets stored in the host memory
itself and offloaded through the BMC.

#### Resource dump
A special type of host dump initiated and collected by the host based on the
request from user. No system state change during the collection of this dump.
A resource indicator is used to indicate what data to be collected. The content
of the dump is decided by the host. This dump get stored in host memory and
