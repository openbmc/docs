...
and exposed to host user space applications. For example, it's impossible to
...
| Term           | Definition                                                                                                                                |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| AC power cycle | Power off both motherboard and BMC then power on them both. Base on the DC-SCM spec, BMC should power on before release host reset signal |
...
The main function for btmonitor is to report the duration for each power cycle
stage. \
...
| Kernel Shutdown    | Host is shutdowning all the kernel processes and releasing all the resources that are owned by the kernel and firmware.                             |
...
and scenarios that might happen and cause server having another unexpected AC/DC
power cycle. This section discussed what impact we would have when an unexpected
power cycle happens under different stages.

| Stage                                                           | Power cycle type | Impact                                                                                                             |
| --------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------ |
| Serving                                                         | DC               | `D_total` = 0* <br/> `D_unmeasured` = 0* <br/> `D_user_down` = 0 <br/> `D_kernel_down` = 0                         |
| Serving                                                         | AC               | `D_total` = 0* <br/> `D_unmeasured` = 0* <br/> `D_user_down` = 0 <br/> `D_kernel_down` = 0 <br/> `D_post_down` = 0 |
| Userspace shutdown                                              | DC               | `D_user_down` = 0 <br/> `D_kernel_down` = 0                                                                        |
| Userspace shutdown                                              | AC               | `D_user_down` = 0 <br/> `D_kernel_down` = 0 <br/> `D_post_down` = 0                                                |
| Kernel shutdown                                                 | DC               | None                                                                                                               |
| Kernel shutdown                                                 | AC               | `D_post_down` = 0                                                                                                  |
| Post shutdown                                                   | DC               | None                                                                                                               |
| Post shutdown                                                   | AC               | `D_post_down` = 0                                                                                                  |
| BMC                                                             | DC               | Impossible scenario. Because the host is power off at this stage.                                                  |
| BMC                                                             | AC               | None                                                                                                               |
| Firmware <br/> Loader <br/> Kernel <br/> InitRD <br/> Userspace | AC & DC          | All the extra time spent will be added to `D_unmeasured`                                                           |

*If host can properly send the `D_total` even unexpected power cycle happens in
the serving stage. Then `D_total` & `D_unmeasured` will still be correct.

## Alternatives Considered

### Can this be done with host notification only?

Sending the notification only will make btmonitor lost the capability of
...
the btmonitor needs to monitor host status to get some timepoints. \
...
btmonitor. I think putting btmonitor into this repo might not be appropriate.
