...
## Problem Description
BMC's can reboot for a lot of different reason. User request, firmware update,
...
The high level flow is that OpenBMC software first checks for pgood, and if
set, it then pings the host to see if it is up and alive. If the power is on and
the host is running, then files are created in the filesystem to indicate this:
- /run/openbmc/chassis@0-on
- /run/openbmc/host@0-on

The obmc-chassis-poweron@.target and obmc-host-start@.targets are then started.
...
systems want to recover automatically (i.e. reboot the host) for the customer.
...
- Both IPMI and PLDM stacks will give the host 30 seconds or less to initially
...
- IPMI will continue to utilize the btbridge SMS_ATN path to indicate to the
  host that a "Get Message Flags Command" is requested. Upon the host issuing
  that command, it will be considered up and running
...
host via IPMI or PLDM, then then BMC will read this processor CFAM register.
...
denominator" hardware register of some sort. The problem with that is your start
...
- Reboot the BMC while the host is booting, but before it's in a state where
  ...
  scenario once it comes back from it's reboot and takes the proper recovery
