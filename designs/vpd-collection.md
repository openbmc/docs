...
- IPZ VPD is required to implement and validate ECC as defined in the OpenPower
  specification (The BMC code does not validate/use ECC although the
  specification does define it).
...
- If a FRU does not have a VPD store such as an EEPROM, the BMC should be able
  to synthesize VPD for such FRUs. Details on VPD syntesis will be in it's own
...
- The sysfs path to the VPD EEPROM (if applicable)
