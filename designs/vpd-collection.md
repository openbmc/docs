...
The BMC also supports just one VPD format, the [OpenPower VPD] [1] format.
...
- A udev [rule] [3] that is used by systemd to launch the above service in
  reaction to the EEPROM sysfs interface being created.

- A set of config [YAMLs] [4] that tell the op-vpd-parser what data from the
  VPD to update into the inventory and into what D-bus
...
- Another service will be created to update location code information into the
  inventory. Since the location code expansion comes from the system VPD, this
  service can only be launched after the system VPD (stored on the backplane) is
  collected.

- There will be one udev rule per EEPROM on the system for which the BMC has to
...
- Each udev rule will be setup to launch an instance of one of the VPD parser
  services (The contents of the EEPROM are known at build time as they are
  system specific)

- The service (one instance of ipz-vpd-parser or keyword-vpd-parser), when
  launched, will read the EEPROM, parse its contents and use YAML config files
  to determine what VPD contents to store in the inventory.

- I am also proposing that the services store the full VPD as a dictionary
  property on the D-bus `{record => {keyword => value}*}*`. This is simply
  because we do not want to maintain a YAML config entry for every single
  keyword that is in the VPD. This also allows us to handle cases where the same
  keyword may appear in different records.

- An alternative approach to the previous point is to define one interface per
  record (ex, VINI record) which can have each keyword as a property
...
- Generation of the udev rules and the YAML configs shall be layered such that
  they can be tweaked on a per-system basis.

### Open topics
Some open questions:

- Some more thought is needed on how concurrent maintenance will be handled.
...
- Design would make the application very specific to a given system or a set of
  ...
  approach of small applications that target a very specific requirement are
  better.
