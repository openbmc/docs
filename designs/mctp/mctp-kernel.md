...
`smctp_addr` and `smctp_type` fields of the address are used; the `smctp_tag`
...
`getpeername()` may be used to retrieve a kernel-allocated tag value where
`MCTP_TAG_OWNER` was specified in the original `connect()`.
