...
Unlike an unrestricted `bind()`, the socket will now only receive incoming TO=1
...
The message buffer does not include the message type byte; this is specified
by the `smctp_type` value. If a message integrity check is to be
included in the transmitted message, it must be provided in the message
buffer, and the most-significant bit of the `smctp_type` must be 1.
...
The `smctp_type` field of the passed address argument will be populated with
the MCTP message type received, including the IC bit. The type byte does not
appear in the returned buffer data. If IC is set, the integrity check follows
the message, and will be included in the received buffer.
