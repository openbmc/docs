...
allow very simple endpoints to run without this process. MCTP endpoints that
require more than just single-endpoint functionality (bus owners, bridges, etc)
would include the control message protocol process.
...
This establishes the local address of the socket. Only the `smctp_network`,
`sctp_addr` and `smctp_type` fields of the address are used; the `smctp_tag`
value is ignored.  Incoming MCTP messages that match the network, address, and
message type will be received by this socket.
...
The `smctp_type` field specifies which message types to receive. A special value
of `MCTP_TYPE_ANY` allows the socket to receive messages of any type. Only the
...
Requesters which connect to a responder will typically use `connect()` to
specify the peer address, with `MCTP_TAG_OWNER` to allocate an owned tag for
outgoing messages.
...
associated with the socket, for future unaddressed sends. Any tag value
allocated as part of the original `connect()` call remains allocated to the
socket - if an additional tag is allocated as part of the send, this does not
affect the original allocation.
...
The `retain` flag indicates to the kernel that the socket should not release
previous tag allocations when a message is sent to a new destination EID. This
causes the socket to continue to receive incoming messages to the old (dest,tag)
tuple, in addition to the new tuple.
...
source address, tag-owner, tag) tuple. The tag value is 3 bits in size.
...
## MCTP message-type specification: using `smctp_addr` rather than protocol ##
