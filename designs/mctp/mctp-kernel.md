...
This process would create a socket of protocol `MCTP_PROTO_CONTROL`, with the
`MCTP_ADDR_EXT` socket option enabled in order to access physical addressing
...
## MCTP message-type sockets: using protocol rather than port ##

This design specifies the message-type of a socket in the `protocol` argument to
...
An alternative would be to specify the message type in a way similar to UDP/TCP
ports, within `struct sockaddr_mctp`.

However, the decision made for this design is to allow userspace to send and
recieve messages *without* having to handle the message-type prefix (and
possible message IC) on every transmitted & received message.

This means that the send()/recv() data formats are now dependent on that message
type; making it more appropriate as a protocol argument rather than a member of
`struct sockaddr_mctp`. The kernel will need a way to properly handle
decapsulation/encapsulation, which *may* be a property of the underlying message
type, so needs support for each specific "protocol".
