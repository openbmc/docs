...
given the design choice above, so it is not
suited to use the protocol argument here.
...
requests - more than one response may be valid for a given request message. In
this case, we would release the tag after the first response is received, and
then have no way to correlate the second message with the socket.
