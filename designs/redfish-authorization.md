...
9. BMC implements a complete privilege registry; that is  
  a. it implements all overrides in the Redfish spec; it may support
   configuring overrides at runtime  
  b. BMC exposes PrivilegeRegistry which represents the current
   configuration and reflects runtime changes   
  c. Changes to resource entities shall be propagated to the current privilege
...
created.  
In Google use cases, these APIs will also be called by a dedicated
