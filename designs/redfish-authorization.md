...
#### Authenticated Name to Redfish Role Mapping

Currently, the only mapping from authenticated name to Redfish role is the
[hardcoded Linux permission to Redfish role mapping](https://github.com
/openbmc/bmcweb/blob/3ba0007367777144f474fdf99439ae8c03633486/redfish-core
/lib/roles.hpp#L28). Google has use cases where authenticated identities are
metadata in X509 certificates which don't have mapping to Linux username
and permissions. This use case can't be supported in OpenBMC when this
proposal is written.

We propose to add an optionality so that BMCWeb can add or remove mappings at
runtime. The mapping will be a string to string table, from identities to
Redfish roles. Existing Linux permission to Redfish role mapping will be
executed before the additional map is looked up. BMCWeb will keep a
predefined maximum number of mappings supported.
...
The identities to Redfish role mapping will be stored separated, possible in
the `bmcweb_persistent_data.json`.
...
2. `add/remove nameToRedfishRole`: BMCWeb takes a string pair, and adds or
removes that from a in memory table.

Any invalid method calls will be rejected, e.g., add a role to an existing
identity, or add a non-existing Redfish role to an identity.
