...
10. BMC implements a complete privilege registry; that is
   ...
   at compile time; it may support configuring overrides at runtime
...
However, modeling roles as Linux groups doesn't worked well with OEM Redfish
Roles where a Redfish role maps to a dynamic set of OEM privileges.

To solve this, we propose the following solution:

**Store Redfish Roles As Linux Users and Secondary Groups**
We propose to store Redfish Roles as both Linux users and groups. Storing as
...
Redfish roles will have a fixed prefix "RfRole". OEM redfish roles will get
prefix "RfRoleOem". For example, the base Redfish role "Administrator" will
result in a Linux secondary group "RfRoleAdministrator" and a local user
"RfRoleAdministrator".
...
"RfPriv". OEM privileges will have fixed prefix "RfPrivOem".

**Username to Redfish Role Mapping**
Mapping a username to Redfish role becomes searching the group starting with
"RfRole" that the user is in.

**Redfish Role to Redfish Privileges Mapping**
Mapping a Redfish Role to Redfish privileges becomes searching all the groups
starting with "RfPriv" of the user.

A user maps be in multiple linux groups meaning they are assigned certain
privileges; for example, user "PowerService" is in "RfRolePowerService" group
meaning its Redfish role is "RfRolePowerService"; "RfRolePowerService" is in
groups "RfPrivConfigureSelf" and "RfPrivOemPowerService" groups meaning
"RfRolePowerService" is assigned to Redfish privileges "ConfigureSelf" and
"OemPowerService".
...
```
 ...
 |                 |<-----------------------|                 |            |group:                      |
 |                 | privileges:            |                 <----------->|  RfRoleOemPowerService     |
 |                 |  Login, ConfigureSelf  |                 |            |                            |
 |                 |  OemPrivPowerService   |                 |            |user: RfRoleOemPowerService |
 +-----------------+                        +-----------------+            |group:                      |
                                                                           |  RfPrivConfigureSelf       |
                                                                           |  RfPrivOemPowerService     |
                                                                           |  RfPrivLogin               |
                                                                           +----------------------------+
```

The above diagram works for LDAP users as well. A remote role or group can
map to base Redfish roles or OEM Redfish roles via RemoteRoleMapping.
...
GROUPADD_PARAM in the useradd class. Base privileges
...
3. Phosphor-user-manager performs validation:
   * Names of OEM Redfish roles are unique and valid; e.g., start with "RfRole"
...
attributes can be generated from the schema at compile time.
...
An authorization operation is still the existing bit-wise `isSupersetOf`
between the required privileges of a given operation on a given resource and
the assigned privileges of a role.
...
will increase the binary size. We shall limit the increase to <= 100KB.
