...
Instead of adding new PLM we can incorporate changes directly to PLDM
But PLDM is trying to decouple all service specific logic from it 
in order to make it a light weight component focusing only on host 
interfacing functionalities. So attempt to add modification related to
license manager in PLDM is discarded.
