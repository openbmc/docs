...
- Events are stored in-memory as an ordered structure like:
  `std::vector<std::pair<uint32_t timestamp, uint32_t code>>`
...
- A runtime configuration will be introduced in `entity-manager` to support
  platform-specific boot progress queue polling and decoding.
