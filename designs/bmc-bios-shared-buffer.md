...
2. The BIOS shall be the main writer to the shared buffer, with the BMC mainly
   reading the payloads, only writing to the buffer to update the buffer header
3. The BMC shall read new payloads from the shared buffer and store it in its
   tmpfs after any processing
4. The BMC shall provide a hook for where a configurable library can be utilized to
