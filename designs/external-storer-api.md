...
is not possible within a typical filesystem. So, a special directory layout
scheme will be used, similar to the
[Redfish Mockup Server](https://github.com/DMTF/Redfish-Mockup-Server). Each URL
will exist as a directory, so that it can contain additional files. The content
associated with that URL will exist within a specially-named regular file in
that directory: `index.json`

This file will contain JSON text. It will be stored within the directory it
applies to. As this name is reserved, a subdirectory with the same name will not
be allowed to be created, as it would conflict. This implies that no URL will be
allowed to have `index.json` as a path component within it, thus no
ExternalStorer instance or entry may be created with that name as the
identifier.
...
$ ls /run/bmcweb/redfish/v1/Systems/system/LogServices/ExampleAlerts/Entries/49ae5069-3375-45b9-adfb-bc16c40ce0a6
index.json
```
...
$ cat /run/bmcweb/redfish/v1/Systems/system/LogServices/ExampleAlerts/Entries/49ae5069-3375-45b9-adfb-bc16c40ce0a6/index.json
...
and `@odata.id` need not be stored, as they are already implied by the directory
