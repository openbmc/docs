...
will also contain the field `Location`, with the same information. Due to schema
requirements, it might have to be moved within another, such as `MessageArgs`.
The intended purpose of the `Location` field is to make the POST operation more
practical for tests and external services to use, which might not have the
ability to easily view headers.
...
The special fields `Id` and `@odata.id` will always be present, automatically
generated if needed. As required by the `LogServices` schema, the special
`Entries` field contains a link to another URL.
...
special field `@odata.id` is present. The customized fields `Id`, `Name`, and
`Description`, provided by the external user during the POST operation, have
different content. This illustrates the difference between the outer layer and
the inner layer. A subtle difference is that, in the inner layer, `Id` is an
ordinary field, customizable just like any other, and is not special.
