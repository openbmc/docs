...
associated with this host. Further GET on this HostEthernetInterfaces property will provide the Interface details for this host.
...
The response will be as per the below redfish DMTF schemas
```
...
```
#### Delete the VMI IP
The HMC can remove the IP address configuration by sending all zeros for the network parameters in the below PATCH request.
...
default parameters. At BMC standby, before the first power on of the CEC, the VMI Interface IP parameters will be set to all zeros.

When the system is powered on initially, HMC should setup the VMI interface to PHYP via BMC. Its recommended to configure single IP address to the VMI since the
PHYP has the limitation to work with only single IP for its interface.

If VMI is configured with multiple IP addresses, the last configured IP will be the active IP.
...
```
Similarly the second interface for the VMI will be under
```
/xyz/openbmc_project/network/host0/eth1
```
and so on based on the number of host interfaces supported by the system.

## Alternatives Considered
1. REST Interface commands to GET and PATCH the host interface parameters.
	- This option is dropped since we have a Redfish schema for the HostInterface configuration.

2. Since this VMI IP setup is specific functionality involving PHYP, BMC and HMC, we could have considered moving it under HMC "specific" URI, like : "/xyz/openbmc_project/HMC/VMInterface"
 	- This option is dropped since we already have the "/xyz/openbmc_project/network/host0/" object at BMC, which can be utilized to save the host interface data.

## Impacts
The existing "/xyz/openbmc_project/network/host0/" object would need modification to remove the current objects such as "intf/addr".

## Testing
1. Verify the interface commands from a redfish client/HMC and verify if the network settings are working as expected.
2. Verify the responses with the Redfish schema validator.
3. Verify the network file is updated when the IP address is configured/modified.
4. Verify the persistence of the IP address configuration by rebooting the BMC.
5. Verify the IP address gets updated with enabled for the DHCP.
