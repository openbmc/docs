...
Google Authenticator provides 'google-authenticator-libpam' PAM module
which supports Time-based One-time Password (TOTP) two-factor authentication for
...
2. GoogleAuthenticator MFA enable is AccountService configuration. This is
   disabled by default
...
will be asked for both its password and a one-time code.  Such one-time codes
can be generated with the Google Authenticator application, installed on the
user's Android device oruse oauth tools.  To respectively generate and verify
those one-time codes, a secret key (randomly generated) must be shared between
the device on which one-time codes are generated and the system on which Google
...
12. BMCWeb needs to pass this MFA TOTP token to pam_authenticate() API along
    with password. google-authenticator PAM module verifies given TOTP token against
    user's secret key to pass authentication successfully.
...
at '/redfish/v1/AccountService/Accounts/<user>'
By default this MFABypass property set to null for each user account.
...
Generate secret key redfish action supports at '/redfish/v1/AccountService/Accounts/<user>'
POST on '/redfish/v1/AccountService/Accounts/<user>/Actions/ManagerAccount.GenerateSecretKey' 
...
```
    User's first login                    BMCWeb                  Phosphor-User-Manager         Linux
  with GoogleAuthenticator                |                              |                        |
   MFA Enabled|                           |                              |                        | 
              ...
              |<--------------------------|                              |                        |
              |                           |                              |                        |
  Create      |   PATCH GenerateSecretKey |                              |                        |
  SecretKey   |-------------------------->|   DBus: CreateSecretKey      | setup secret key file  |
