...
and then expose `xyz.openbmc_project.DeviceSlotPresence` dbus interface of its
own if it detects the hardware.
...
`xyz.openbmc_project.DeviceSlotPresence` interface and entity-manager can have a
...
```json
{
  ...
  ...
  "Name": "My Fan Board 0",
  "Probe": "xyz.openbmc_project.DeviceSlotPresence({'Name': 'fanboard0'})",
  "Type": "Board",
}
```
...
dbus-run-session and create the dbus interfaces expected from the other daemon
from within the test, to create the expected environment.
