...
* Support all binding compiled in to a single (MCTP Daemon) application
* Execute separate instance (of MCTP Daemon) for each physical interface.
...
medium.
* Start MCTP Daemon as user space application, which will be exposing
D-Bus objects for the MCTP devices discovered.
* Binding support (SMBus, PCIe) and control commands will be added to libmctp.
* Entity-manager will be used to advertise the configuration required, which
...
* PLDM, SPDM, PCI VDM, NVME-MI application will run as separate daemon
...
with a large amount of platform-specific customisation's. This also does
