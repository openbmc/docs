...
Vision is to make the MCTP communication through socket based approach,
but due to the current limitations on advertising MCTP discovered
devices & it's properties in standard way, D-Bus approach is used (which
provides a standard way already). Based on the usage compile time flag (TBD)
option in application layer (Wrapper library) can be used to have a easy
switch.
...
provides the packetization and routing functions, between:
 ...
 - an "upper" messaging transmit/receive interface, for tx/rx of a full
   message to a specific endpoint (i.e., (1) above)
 ...
 - a "lower" hardware binding for transmit/receive of individual
   ...
   hardware, and defines the parameters of the common packetization
   code (i.e. (2) above).
...
license for the library should also allow this re-use; a dual Apache &
GPLv2+ license may be best.
...
All the proposed D-Bus interfaces are for reference, and will be updated
in this document as we go.
...
|----------------------|----------------|--------------------------------------|
|  Eid                 | byte           | 8-255                                |
|  NetworkId           | uint16         | MCTP network it belongs              |
|  BindingTransportId  | enum           | As per DSP0239 Table 3               |
|  MediumId            | enum           | Physical Medium specific             |
|  StaticEid           | boolean        | Static / Dynamic EID support         |
|  UUID                | array[byte][16]| GUIDs                                |
|  SendMessage         | method         | params:Dst EID,MsgTag,TO,payload/fd  |
|  RecvdMsg            | signal         | arg-MsgType,srcEID,MsgTag,TO,payload |
|  BindingMode         | enum           | Bus Owner, endpoint device           |
|  BaseTransmissionUnitSize | uint8_t   | Baseline Transmission Unit           |

##### xyz.openbmc_project.MCTP.SupportedTypes

Interface used to represent the supported MCTP types. This will be exposed
by both Base object (for both Bus Owner & Device endpoint) and in all the
discovered MCTP devices (in case of Bus Owner MCTP daemon)

| Method/Property name |      Type      |        Applicable Values             |
|----------------------|----------------|--------------------------------------|
|  MCTPControl         | boolean        | Indicates support availability       |
|  PLDM                | boolean        | Indicates support availability       |
|  NVMeOverMCTP        | boolean        | Indicates support availability       |
|  SPDMOverMCTP        | boolean        | Indicates support availability       |
|  PCIeVendorDefined   | boolean        | Indicates support availability       |

Note:
1. Other MCTP message types will be added as and when required.
2. Defined the Message type as `boolean`, so that it will be easy for the
application to register based on the need, and accordingly, MCTP daemon
can advertise the supported types.

##### xyz.openbmc_project.MCTP.BusOwner

Exposed by MCTP daemon, which runs in Bus owner mode.

| Method/Property name |      Type      |        Applicable Values             |
|----------------------|----------------|--------------------------------------|
| EidPool              | array[struct[byte, byte]]| allowed EID range to be used|
| TopMostBusOwner      | boolean        | Indicate this is Top most bus owner  |
| OwnEidPool           | boolean        | Indicates Eid pool is managed by self|
| DiscoverDevices      | boolean        | Indicates support to do device discovery|

##### xyz.openbmc_project.MCTP.Device

Interface exposed by MCTP daemon on each discovered MCTP devices.
...
| Method/Property name |      Type      |        Applicable Values             |
|----------------------|----------------|--------------------------------------|
| ArpMasterSupport     | boolean        | ARP support                          |
| BusNumber            | byte           | i2c bus number of the medium used    |
| SlaveAddress         | byte           | Slave address to be used for this medium|

##### xyz.openbmc_project.MCTP.Binding.PCIe

This interface has properties/methods/signals specific to the PCIe binding.

| Method/Property name |      Type      |        Applicable Values             |
|----------------------|----------------|--------------------------------------|
| DiscoveredFlag       | enum           | Flag is set when received Set EID cmd|
| BDF                  | uint16         | BusNumber DeviceNumber FunctionNumber|

[TBD for other bindings]
...
with a large amount of platform-specific customization's. This also does
...
HTTP client, a TCP/IP stack, a JSON (de)serialize, and support for
...
Redfish-over-MCTP channel (DSP0218), which uses simplified serialization
