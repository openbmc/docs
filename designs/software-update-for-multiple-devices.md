...
sub-string such as "BIOS","CPLD", "VR", "BIC" (both upper and lower cases) just
...
type. Besides *dots*, *underscores* and *-* are also considered
...
Example : If wanted to update two different BIOS for two hardware revisions. So,
we can use this compatible string and check the type as Host.Bios and revisions
as EVT/DVT/PVT. If the EVT/DVT/PVT has revisions, we can have EVT1, EVT2, etc,.
...
Systemd service which is invoking the flash tool will have its status to failed
or inactive status if any errors or update fails. Based on these systemd service
states, ItemUpdater can get the error status.
...
If we try to copy new image for FW update, then BMC may go out of storage. To
overcome this BMC out of storage issue, If new/another FW update is not allowed
when "AutoDelete" flag is set as "no". Firmware update is allowed only if
"AutoDelete" flag is set as "yes".
