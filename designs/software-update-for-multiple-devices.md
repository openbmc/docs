...
Example : If wanted to update two differnt Bios for two hardware revisions. So,
we can use this compatible string and check the type as Host.Bios and revisions
as EVT/DVT/PVT. If we EVT/DVT/PVT has revisions, we can have EVT1, EVT2, etc,.
...
With Different Harware Revisions, we can handle different BIOS methods.
...
systemd service which invokes the flash tool status will be updated with failed
...
Example:
   ...
   >
   > Compatible=com.facebook.Software.Element.TwinLake.Type.Host.Bios.Rev
   >
   >
