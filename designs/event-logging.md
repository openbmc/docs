...
The `Foo.errors.yaml` content will be expanded to include the content formerly
in the `Foo.metadata.yaml` files specified by `phosphor-logging`. A new file
type `Foo.events.yaml` will be defined, which will cover both the current
`error` and `metadata` information as well as augment with additional
...
While it would have been desirable for `sdbusplus` to generate a Redfish Message
Registry directly, this is not possible if we would like to support mapping a
`Logging.Entry` event to an existing standardized Redfish event (such as those
in the Base registry). The generated information must contain the
`Logging.Entry::Message` identifier, the `AdditionalData` to `MessageArgs`
mapping, and the translation from the `Message` identifier to the Redfish
Message ID (when the Message ID is not from "this" registry). The last part
precludes directly using the Redfish Message Registry format (with some OEM
extensions for the translation information).
