...
generating and running unit tests annotated with `#[test]` via `cargo test`, and
has an extensive third-party package repository hosted on [crates.io][33], as
well as support for referencing git repositories directly.
...
One interesting examination of Rust binary size is [Writing a winning 4K into in
Rust][34], which reviews the changes necessary to translate a size-optimized C
program to Rust.
