...
Other contributors: Nhat Hieu Le (lhieu@os.amperecomputing.com), Thang Q.Nguyen,
Andrew Jeffery (andrew@codeconstruct.com.au), Quang Nguyen, Prithvi Pai
(ppai@nvidia.com),
...
### Host retrieve BMC IP address through IPMI commands in EDK2

After receive bootstrap account from BMC, the host need to know the BMC IP
address in redfish host inband interface to send the redfish requests.
...
The subsection "IPMI Commands that Used to Build up Redfish Host Interface" also
mentions which IPMI commands EDK2 will use such get "Get Channel Info", "Set LAN
Configuration Parameters", "Get Network Info"... `phosphor-host-ipmid` needs
support those commands if it does not.
