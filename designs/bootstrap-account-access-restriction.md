...
BMCWeb supports multiple instances, one instance can be added to only response
for `redfish host interface` requests use
...
80 with the inband redfish requests. Example: Add `additional-ports=199` and
`additional-bind-to-device=usb0` to specify that the request to interface `usb0`
should using port 199. BMCWeb will base on these options to create new BMCWeb
instance.

#### Add `support BootStrap account` option

BMCWeb supports multiple instances, one BMCWeb instance will be bind to
`redfish host interface` device. But to identify which instance supports
authenticate use `bootStrap account`, the additional option
`additional-support-bootstrap` is be added. Only the BMCWeb instance with this
option is `true` will response for the redfish host interface request with
bootStrap username.

```text
option(
    'additional-support-bootstrap',
    type: 'array',
    value: [],
    description: '''Allows specifying support bootstrap account
                    directive for each additional socket file''',
)
```
