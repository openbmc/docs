...
ethernet devices `eth1` and `eth2` respectively.
...
Adding a user group name `obmc-rfr-bootstrap`. All bootstrap accounts when
created are categorized into `obmc-rfr-bootstrap` group.
...
Each platform shall have a pre-configured ethernet interfaces which are
dedicated for host interface of hosts. These ethernet interfaces need to be
configured in json configuration file of `entity-manager` and an udev rule shall
be added to ensure that all of these ethernet interfaces shall not be changed
randomly during interface bring up time.

DSP0270, section 9, specify the implementation of IPMI command to create
...
BMCWeb , on startup and when receiving signals which indicates configurations
...
`phosphor-user-manager` is required to register to detect signals indicating
that the host is transitioning to off, in order to delete bootstrap accounts.
...
- Instead of creating a new `obmc-rfr-bootstrap` user group, we can add a dbus
...
- Considering the device naming of udev for ethernet interfaces may keep
  ...
  distinguish host interface connection. But since the code implementation of
  the proposed design which relies on IP address to distinguish request from
  host is more straightforward, it was not selected.
...
- `phosphor-user-manager`, create a bootstrap account and ensure that the
  account has the value `bootstrap-rfr-bootstrap` in its `UserGroups` property.
