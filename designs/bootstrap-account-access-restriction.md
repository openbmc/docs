...
- Update the `Enabled` property in `HostInterface.CredentialBootstrapping` D-Bus
  interface which can be hosted by phosphor-user-manager.
...
requests use
[`additional-ports` and `additional-bind-to-device` options](https://github.com/openbmc/bmcweb/blob/127afa70bfa236b2949f5c385704e00de99557e4/meson.options#L369).
...
- `RoleId`: The role used for the bootStrap account.
...
80 with the inband Redfish requests. Example: Add `additional-ports="440"` and
`additional-bind-to-device="usb0"` to specify that the request to interface
`usb0` should using port `440`. BMCWeb will base on these options to create new
BMCWeb socket.

#### Add `support BootStrap account` option for specific port

BMCWeb supports multiple sockets, and one BMCWeb socket will be bound to the
`redfish host interface` device. But to identify which socket supports
authenticate use `bootStrap account`, the additional option
`additional-support-bootstrap-credential` is be added. Only the BMCWeb instance
with this option is `enabled` will response for the Redfish host interface
request with bootStrap username.

```text
option(
    'additional-support-bootstrap-credential',
    type: 'array',
    value: [],
    description: '''Allows specifying an bootStrap credential support option
...
Ipmi -> User: [Step 2.4] Update "Enabled" property of HostInterface.CredentialBootstrapping D-Bus interface base on\n "Get Boot Strap Account" request
...
Web -> Web: Go to next step
end
Web -> Web: [Step 6.2] The request target to \n+ The port and IP address of Redfish Host Interface?
alt No
Web -> Host: Unauthenticated
else Yes
Web -> Web: Go to next step
end
Web -> Web: [Step 6.3] Is bootStrap account?
alt No
Web -> Host: Unauthenticated
else Yes
Web -> Web: Go to next step
...
- Add `additional-support-bootstrap-credential` option to `BMCWeb` to configure
  the socket support Redfish Host Interface.
- Add two APIs `ipmiUserSetUserGroups` and
  `ipmiUserSetUserBootStrapAccountState` in `phosphor-host-ipmid`.
- Add `credential_bootstrapping_enable_after_reset` option to
  `phosphor-user-manager` to enable/disable credential bootstrapping after BMC
  reset.
- Redfish Host Interface needs to be bond to specific port and network interface
  such as port 440 and `usb0` in the example.
...
- BMC will only respond `success` for the in-band Redfish request with the IP
  Address of `usb0` interface at port 440 and bootStrap accounts.
...
- Inband Redfish request + bootStrap account + port is not 440
