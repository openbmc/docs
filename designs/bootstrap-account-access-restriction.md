...
- A methodology to send the BMCIp in Redfish host interface to Host.
...
## Send BMCIp in Redfish host inband interface to Host
...
In the redfish host inband interface, BMC is DHCP server and the host is DHCP
client, A BMC can assigns IP addresses and network configuration to host through
a four-step process: Discover, Offer, Request, and Acknowledge (DORA).

### SMBIOS Type 42

Section 8 "SMBIOS support" in `Redfish host interface` specification detail the
SMBIOS tables which BMC can use to send the BMC Ip address in redfish host
inband interface to host. This requires BMC support SMBIOS configuration.
