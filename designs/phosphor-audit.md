...
```ascii
 ...
 |         +-------v--------+    +--------------------------------------+   |
 |         |                |    |                                      |   |
 |         | rc=handle(cmd, +--->|  audit_event<type::NET_IPMI>         |   |
 |         |          input)|    |       (rc,cmd,user,host,user_data="")|   |
 |         |                |    |                                      |   |
 |         +----------------+    +--------------------------------------+   |
 |                 |                                |                       |
 |         +-------v--------+                       |                       |
...
* The filtering
 ...
 > Possible list of commands/paths/etc which have to be filtered and processed.
...
The unifed model for reporting accidents to the end user, where the transport can be:
