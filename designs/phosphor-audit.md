...
The application sends 'method_call' to audit service via D-Bus. What is happening
...
or we can do all of the above and all of this can be optional. If any other
service wants to get any additional information about audit state or actions
, which are made by some service, for preventing races and other problems which
'signals' may bring it should make a request to the audit service about such
action.
...
commands (blacklist or filter) and status of its service (disabled or enabled).
 ...
 * client's layer
    ...
    2. checks if audit event should be filtered or blacklisted
