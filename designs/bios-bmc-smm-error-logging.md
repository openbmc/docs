...
* `libmctp` and MCTP PCIe VDM were considered.
  * `libmctp`'s current implementation relies on LPC as the transport binding and
    IPMI KCS for synchronization. LPC as discussed, does not fit our current need
    and synchronization does not work.
