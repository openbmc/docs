...
- The target SMC, which should refer to a
  ...
  to figure out the correct address to send Redfish request. It also serves as
...
it will check the activation condition against entities discovered by entity
manager to determine whether it should be activated or not. It will also monitor
addition/removal of certain entities to activate or deactivate configeration
dynamicly.
...
`xyz.openbmc_project.Inventory.Decorator.Compatible` appearing or being added
within the system. Additionally, the daemon will look for the exposed
[SatelliteController](https://github.com/openbmc/entity-manager/blob/e25a3890392093fec8f49b84c5bbce99055c6768/schemas/satellite_controller.json)
with name `HMC`. If found, the `HostName` and `Port` will be used for setting up
HTTP connection later on. If not found, the configeration will remain
deactivated.

Once the configeration becomes activated, its corresponding tasks will start
running. For example, the `"Sensors"` part of the configeration will trigger the
following task run every `200ms`, configured by `IntervalMs`:
