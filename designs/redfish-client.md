...
`boost::asio::awaitable<T>` driven by
`boost::asio::io_context`, while sdbusplus uses stdexec's sender/receiver
pattern with `exec::task<>` driven by `execution::run_loop`. These two coroutine
types are not interchangeable — a `boost::asio::awaitable<T>` cannot be
`co_await`ed inside an `exec::task<>`, and vice versa. To bridge the two, one
would need to run a separate `io_context` on an additional thread and use an
`eventfd` with `sdbusplus::async::fdio` to signal completion across event loops.
This adds an extra thread for Asio's event loop, requires manual signaling to
bridge completion across event loops, and introduces thread-safety concerns when
shuttling results between threads. In contrast, libcurl with
`curl_multi_waitfds` integrates naturally with sdbusplus by providing file
descriptors that can be monitored directly via `sdbusplus::async::fdio`,
requiring no extra threads or event loop bridging. A downstream implementation
reference of integrating libcurl with sdbusplus can be found at
