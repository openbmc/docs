...
it is built on top of Boost.Asio, which uses a different coroutine framework
than the stdexec framework employed by the sdbusplus/async library. As a result,
integrating both libraries into a single application may present challenges.
Therefore, it is preferable to avoid such integration if a simpler solution that
already meets the requirements is available.
