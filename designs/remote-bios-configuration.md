...
6. Design document for
   [Prerequisite](https://gerrit.openbmc.org/c/openbmc/docs/+/79327)
...
3. Once all the attributes are received, then bmcweb will convert JSON data into
   DBus format and then it will be exposed to DBus via bios-config-manager on
   "xyz.openbmc_project.BIOSConfig.Manager" DBus interface and update the
   "BaseBIOSTable" DBus property.
4. The "BaseBIOSTable" DBus property expects the input in the BIOS Attribute
   Registry format as specified in
   [bios-config-manager-pdi](https://github.com/openbmc/phosphor-dbus-interfaces/blob/master/yaml/xyz/openbmc_project/BIOSConfig/Manager.interface.yaml#L62)
