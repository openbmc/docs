...
- Both BMCs must report their role via Redfish. The active BMC must also report
  if redundancy is enabled.  The BMC driving the failover must also report that
  a failover is in progress while it transitions to becoming active.
...
failover. It is only available on the passive BMC, and [will be available on
Redfish](#redfish-and-the-host-interface). It must on the passive BMC because
the active BMC may be dead at the time. The force parameter can be used to have
the passive BMC still failover in cases where it usually wouldn't.
...
services.  It will do this by starting the obmc-bmc-active.target to start up
all of the active only services, and then doing a full file sync and enabling
...
redundancy couldn't be enabled, then the syncing and RedundancyEnabled
