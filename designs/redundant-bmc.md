...
- There are conditions that would exclude a BMC from being chosen as the active
  ...
  It's possible that if both BMCs have issues that neither BMC could be active
...
To start with, a new target for each role will be created:
`obmc-bmc-active.target` and `obmc-bmc-passive.target`.

These targets will not be started as part of multi-user.target/BMC ready. This
...
```sh
$ cat phosphor-broadcast-active-bmc-role.service
[Unit]
Description=Broadcast BMC role in /run/openbmc
...
4. The new service and conf files will reside within phosphor-state-manager and
   ...
   enabled.
5. There is also an obmc-bmc-passive.target that will be started when the
   passive role is obtained. This is so a service can be started as part of the
   passive target.

[This commit][rbmc commit] accomplishes the above:
...
2. A recipe or bbappend that provides a service that is only for the active BMC
   would inherit the redundant-bmc bbclass and then specify the service by
   adding it to RBMC_ACTIVE_ONLY_SERVICES:${PN}.
3. When the machine feature is enabled the bbclass will add a post do_install
   function to:
...
- mapper-wait on any of the above services
...
The following properties are on the `xyz.openbmc_project.State.BMC.Redundancy`
interface:

- `Role`
  - Reflects the current role and will also be available via Redfish.
- `RedundancyEnabled`
  - Reported via Redfish to let external users (the web UI and management
    console) know that the system has redundancy operational. The passive BMC
    obtains this from the active BMC.
- `DisableRedundancyOverride`
  - Used to manually disable redundancy on the active BMC.
- `FailoversAllowed`
  - Indicates if a failover is currently possible.
- `FailoverImminent`
  - Set after a call is made to start the failover so that the other BMC can
    react to the upcoming failover.
- `FailoverInProgress`
  - Indicates a failover is currently in progress. It only gets set to true on
    the original passive BMC, and stays true until it becomes operational as an
    active BMC.

The YAML definition has the full [details][1].

On the same object path is an interface to interact with failovers:
`xyz.openbmc_project.Control.Failover`.

It has:

- `StartFailover`
  - A method to kick off a failover. It takes a map of options and a Requester
    parameter that identifies who is calling it.

The YAML definition has the full [details][2].

The passive BMC will drive the failover when its `StartFailover` method is
called. For now at least, the active BMC will reject any `StartFailover` calls.
...
It would provide a D-Bus API at `/xyz/openbmc_project/state/bmc1` that the RBMC
state daemon uses to retrieve information from the sibling.
...
return errors, or the interface could be removed from D-Bus altogether.
...
| CurrentBMCState   | The BMC State      | State.BMC (State Mgr App)            |
| Version           | Firmware version   | /etc/os-release                      |
| Heartbeat         | Heartbeat status   | Signal from State.Red.BMC (RBMC App) |
| FailoversAllowed  | Failovers allowed  | State.Redundancy.BMC (RBMC App)      |
| FailoverImminent  | Failover imminent  | State.Redundancy.BMC (RBMC App)      |

These properties are spread across the following interfaces:
...
- Role: So it can tell if there is already an active or passive BMC when it
  comes up.
- RedundancyEnabled: Indicates there are no issues with the redundancy and data
  is being synced.
- Provisioned: So it knows why network communication doesn't work.
- CurrentBMCState: If the passive BMC is Quiesced, then the active BMC knows
  that redundancy shouldn't be enabled.
- Version: The active BMC will not enable redundancy if the code levels don't
  match. This doesn't use the version from the software updater app because that
  hashes in the flash side.
...
- FailoversAllowed: If a failover is possible in this instance. Aside from
  redundancy being enabled there can also be other requirements like the system
  isn't in the middle of a boot.
...
authentication configured) or if there is an issue with the network hardware.
...
On a POR, the RBMC app will first set its role on D-Bus to 'Unknown' and then
commence determining the actual role, which may take some time. There may be a
condition that requires it to become passive, in which case it will set that
without needing to wait for the other BMC to come up.
...
For redundancy to be enabled, the following must be true:

- The passive BMC is responding and in the Ready state.
- The BMCs must have the same code levels.
- The network between the BMCs must be provisioned and communication possible.
- [DisableRedundancyOverride](#d-bus-representation) hasn't been set.

#### Becoming passive

The passive BMC just needs to start the obmc-bmc-passive.target target.

#### Failovers

The flow of a failover looks like:

1. Set `FailoverImment` to true and pause for a set amount of time, 10 seconds
   to start, so the other BMC can react to that indication.
1. Set `FailoverInProgress` to true.
1. Set `RedundancyEnabled` to false.
1. Set `FailoverImment` to false.
1. Block BMC reboots.
...
1. When the new passive BMC is back, enable redundancy if possible. Assuming it
   can be enabled, do a full sync and enable background syncing.
...
The [Redundancy schema][] will be used to handle the redundancy related information.
It doesn't yet handle the concept of a role, so that will be worked with the DMTF.
...
- phosphor-logging
  ...
  - Have a unique event log Id property range per BMC, so that there is no
    ...
    each. This could be done by having the BMC position encoded into the first
    byte of the 4 byte BMC log ID. This would be a compile option. Logs are
    required to be synced so that if a BMC dies any logs it had created first
    can be found on the other. Deletes would also need to be synced, in both
    directions.
...
The systemd service override files and the new active target files can also be
stored in the phosphor-state-manager repo.
...
[Redundancy schema]: https://redfish.dmtf.org/schemas/v1/Redundancy.v1_5_0.json
...
[1]: https://gerrit.openbmc.org/c/openbmc/phosphor-dbus-interfaces/+/70765
[2]: https://gerrit.openbmc.org/c/openbmc/phosphor-dbus-interfaces/+/78371
