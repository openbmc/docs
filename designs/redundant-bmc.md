...
`phosphor-rbmc-state-manager`, that runs on each BMC to handle determining
roles, redundancy D-Bus APIs, etc.
...
| RedundancyEnabled | bool   | Redundancy enabled     |
| Provisioned       | bool   | If already provisioned |
| BMCState          | enum   | The BMC State          |
| PassiveRequested  | bool   | Request to be passive. |
...
scenarios, and not as useful in the use case where the BMCs don't automatically
failover themselves. It would also take a lot of new code to do things like
determine which BMC should be active based on custom rules. Other disadvantages
are:
...
- The BMC does not need to trigger a failover if any service is failed to start.
  By default, the pacemaker just moves the failed service from one node to
  another node. It required a lot of configuration to prevent.
