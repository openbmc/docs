...
1. A network cable.
2. FSI cables. FSI stands for `FRU Service Interface` where FRU stands for
...
- A specific IBM requirement is that an active BMC still needs to be chosen and
...
4. The new service and conf files will reside within phosphor-state-manager and
   be installed during a bitbake build.

The commit proposed [here][rbmc commit] accomplishes the above by:

1. Providing an RBMC_ACTIVE_ONLY_SERVICES bitbake variable to add active-only
   services into. A phosphor-state-manager bbappend in a system layer could then
   add in the service names as desired.
2. Installing the active-bmc-only.conf file mentioned above into the
   `<service>.d` subdirectory for that service. This is done from the
   phosphor-state-manager recipe when the 'redundant-bmc' packageconfig is
   enabled.
3. Linking that service into the obmc-bmc-active.target.wants directory so it
...
[will be available on Redfish](#redfish-and-the-host-interface). It's only on
...
| ----------------- | ---------------------- | ------------- |
| Role              | Determined Role        | RBMC App      |
| RedundancyEnabled | Redundancy enabled     | RBMC App      |
| Provisioned       | If already provisioned | TBD           |
| BMCState          | The BMC State          | State Mgr App |
| FWVersion         | Firmware version       | SW App        |
| Heartbeat         | Heartbeat status       | RBMC App      |
...
interface `xyz.openbmc_project.State.BMC.Redundancy.API` at
`/xyz/openbmc_project/state/bmc1` with a property for each value in the table.
How the data is acquired is hidden in the daemon. The RBMC state daemon would
then read the properties to get the sibling values, and under the covers the
communication daemon could return a buffered value, or possibly read the sibling
at that time. It could listen for PropertiesChanged signals to know when a value
changes.

If the sibling isn't there or doesn't respond, reading the properties would
return errors.

The daemon uses the single `Redundancy.API` interface and not instances of the
source interfaces - `xyz.openbmc_project.State.BMC`,
...
syncing ([data sync design][data sync design doc]), and then starting the
obmc-bmc-active.target to start up all of the active only services. It will also
set the 'RedundancyEnabled` property to indicate it is ready for failovers. If
redundancy couldn't be enabling, then the syncing and RedundancyEnabled
assertion would be skipped.
...
- phosphor-logging
  ...
  - Have a unique log ID range per BMC, so that there is no collision with logs
    created by the other BMC as the logs will be synced to each. This could be
    done by using the first byte in the uint32_t log ID as the BMC identifier.
    This would be a compile option. Logs are required to be synced so that if a
    BMC dies any logs it had created first can be found on the other.

  - When logs are synced via their files, phosphor-log-manager will need to
    detect the new files and put them on D-Bus so both BMCs can always show all
    event logs.
...
[rbmc commit]: https://gerrit.openbmc.org/c/openbmc/openbmc/+/70957
[chrony NTP server]: https://gerrit.openbmc.org/c/openbmc/openbmc/+/71716
[service config design]: https://gerrit.openbmc.org/c/openbmc/docs/+/71441
[DRDB]: https://en.wikipedia.org/wiki/Distributed_Replicated_Block_Device
