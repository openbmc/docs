...
4. The new service and conf files will reside within phosphor-state-manager and
   be installed during a bitbake build if the redundant bmc machine feature
   is enabled.
...
2. When the machine feature is enabled the bbclass will add a post do_install
   function to:
...
daemon could return a buffered value, or possibly read the sibling at that
time. PropertiesChanged signals would be emitted to know when something
changes.
