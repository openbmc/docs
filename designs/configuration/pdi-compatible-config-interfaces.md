...
- **Interface explosion**: Arrays of objects generated interfaces like
...
sorted by subjectively perceived importance
...
- Updating the config schema can also be easily handled by using versioned DBus
  interfaces. The `GetManagedObjects` function can be used to find the
...
- Easily supports optional properties
- varlink already uses json in IPC
...
description:
  interface xyz.openbmc_project.Configuration.SPIFlash
  The 'FirmwareInfo' property has fields 'VendorIANA' and 'CompatibleHardware'.
  The 'MuxOutputs' array elements have fields 'Name' and 'Polarity'
properties:
  - { name: SPIControllerIndex, type: uint32 }
  - { name: SPIDeviceIndex, type: uint32 }
  - { name: FirmwareInfo, type: struct[uint32, string] }
  - { name: MuxOutputs, type: array[struct[string, enum[Low, High]]] }
```
...
Alternative 5 (Typed Containers, for objects in arrays only)
...
- Implement and run a one-time script to generate PDI yaml from EM config
  schema. This is not a process change but a one-time thing to get the PDI yaml
  for all the EM schemas which are compatible at this point.
...
- The design intent is not to force PDI on anyone's repo, but make it possible
  to use it for interactions with configuration interfaces.
- The reasoning here is to avoid consuming reviewer's limited time with changes
  not strictly required to implement this design.
