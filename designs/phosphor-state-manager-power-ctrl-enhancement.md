...
```
 ...
 - Add support for OS (Operating System) state change, Based on the
   post complete.
...
```
 ...
 - The operating system state change will be updated based on the post complete.
   which is added additionally in the phosphor-state-manager to determine OS
   state change.

 - A separate script will monitor and read the post complete status for each
   hosts. If the status is true the OS state will be standby or else inactive.
