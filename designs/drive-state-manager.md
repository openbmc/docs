...
Author: Eric Yang <eric.yang.wiwynn@gmail.com>
...
1. Provide per-drive power control (power cycle, reboot, hard reboot) via a
   D-Bus interface that bmcweb and other consumers can use.
...
signals for the `poweron` and `poweroff` state targets only (not the marker
targets), matching the chassis state manager pattern.
...
- Unit tests for the transition-to-target mapping and for unsupported transition
  handling (`NotSupported` throws `NotAllowed`).
- Integration test: set `RequestedDriveTransition` via D-Bus and verify the
  correct systemd target is started.
- Redfish test: verify `Drive.Reset` POST returns the correct D-Bus property
  change and that the action is absent when no State.Drive object exists.
