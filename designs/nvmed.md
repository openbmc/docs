...
3.  [Management Component Transport Protocol (MCTP) Base
...
In addition, the NVM Express Management IF Workgroup also published the [NVM
Express Management Interface Specification][nvme-mi] for out-of-band (and inband
tunneling which is out of scope for this doc) management. The Management
...
- The performance will be evaluated from the following aspects:
  ...
  - Delays across multiple queries, e.g. delays on thermal sensor while a large
    log transfer.

  describes the NVMe Basic Management Command and is included here for
  informational purposes only. The NVMe Basic Management Command is not formally
  a part of this specification and its features are not tested by the NVMe
  Compliance program." host software", Page 107, NVM Express Management
