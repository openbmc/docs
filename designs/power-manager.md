...
application. This application has 2 main features: `Power Monitoring` and `Power
Limit`.
...
The properties of the `Power Limit` feature are defined in [Power Cap][power-cap]
interface.
...
Management` application shall log an event if the exception action is _Hard Power
Off_ or _Log Event Only_ to inform users the status of the system's power.
