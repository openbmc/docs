...
total power consumption without hardware dependency. When the total power
consumption exceeds the system limit, the system can be broken. To protect the
...
commands. Those commands have 2 main features: Power Monitoring and Power Limit.
...
- System Power Statistics: Each system only has 1 option for _System Power
  Statistics_ mode. The unit of timeframe in this option is microseconds.
- Enhanced System Power Statistics: The system can have multiple options for
  _Enhanced System Power Statistics_ mode. The unit of timeframe in this option
...
- Power monitoring sensors shall be updated at an average rate of at least once
  per second. If the BMC can not get the sensor value each second, the old
  sensor's value shall be assumed this is current value.
...
```
    ...
    +------------------+    |         |    +------------------------+
                        ...
                        +-----------------+
                            ...
                            |         |
    +------------------+    |         |    +---------------------+
    |  phosphor-host-  |----+         +----|       bmcweb        |
    |       ipmid      |                   |                     |
    +------------------+                   +---------------------+
```

The total power consumption of the system can be indicated by Dbus-sensors or
virtual-sensors application, the `Power Management` get signals of this sensor
to update statistics and check the total power consumption is greater than
system's limit or not. If the total power consumption exceeds the system's
limit, the `Power Management` can request to turn off the host via the
phosphor-state-manager application.

Users can transfer their configuration to `Power Management` via
phosphor-host-ipmid (`ipmitool`) and bmcweb (`curl`).
...
| --------------- | ------ | ----------------------------------------------------------------------------------------------------------------- |
| CorrectionTime  | uint64 | Minimum amount of time to wait before the Exception Action will be taken in microseconds                          |
| ExceptionAction | enum   | Exception Actions, taken if the Power Limit is exceeded and cannot be controlled within the Correction time limit |
| PowerCap        | double | Power cap (system's limit) value in watts                                                                         |
| PowerCapEnable  | bool   | Enable/disable Power Limit feature                                                                                |

The value of **ExceptionAction** property can be one of below options:
...
The `/xyz/openbmc_project/power_manager/power_monitor` object path includes
`xyz.openbmc_project.Control.Power.StatisticSampling` interface which defines
Statistics Sampling period in microseconds. The properties of this interface are
defined at below:

| Property       | Type   | Description                                |
| -------------- | ------ | ------------------------------------------ |
| SamplingPeriod | uint64 | Statistics Sampling period in microseconds |
...
| Property | Type   | Description                                       |
| -------- | ------ | ------------------------------------------------- |
| Duration | uint64 | Statistics duration in microseconds               |
| Mode     | enum   | Standard or Enhanced System Power Statistics mode |
...
In the phosphor-host-ipmid and bmcweb, they implemented power-limit feature, but
it is not enough. Therefore, we have to update phosphor-host-ipmid and bmcweb to
fully support power limit feature. Please refer to below link to get more
information about implementation of phosphor-host-ipmid and bmcweb.
...
Configure properties of `xyz.openbmc_project.Control.Power.Cap` interface, then
check the behavior of BMC when total power consumption exceeds the power limit.
The BMC's behavior depended on exception action property:
...
Monitor the sensor which presents total power consumption, calculate the max,
min, average of this sensor. Compare those value which the results from the
current, maximum, minimum and the average properties of
`xyz.openbmc_project.Control.Power.Monitor` interface.

### Integration Testing Plan

#### Power Limit

Using `ipmitool` to configure `Power Limit` properties:

- `ipmitool dcmi power set_limit <option> <value>`

#### Power Monitoring

Using `ipmitool` to read current, max, mix, average values of total power
consumption:
