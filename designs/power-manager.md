...
The [Telemetry][telemetry-repo] provides a solution for monitoring
Max/Min/Avg/Current values of sensors. When users create a report,
[Telemetry][telemetry-repo] creates a buffer to store reading values. If we use
the Telemetry to monitor total power consumption with the multiple time frame
options (1 day, 2 hours, 3 days ...), so many buffers will be created to store
reading values. In the power-manager, it creates one buffer to support multiple
time frame options.

Beside that, in the Telemetry, the minimum of Interval is 1000ms (but in
Power-monitoring, we expect the maximum of Interval is 1000ms).
