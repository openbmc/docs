...
- D-Bus properties of interface `xyz.openbmc_project.MachineContext` may be
...
- Entity-Manager probes that reference property names from the
  `xyz.openbmc_project.MachineContext` interface (YAML documentation pending) on
  D-Bus can key off GXP hardware data and react accordingly.
...
1. xyz.openbmc_project.Decorator.\* //"doesn't represent anything about the
   physical board" (-Ed Tanous)
2. xyz.openbmc_project.FRUDevice //The data is not coming from a FRU device

#### Proposed interfaces (rejected):

1. xyz.openbmc_project.DeviceTree
2. xyz.openbmc_project.uBootHWID
3. xyz.openbmc_project.GXPFRU //above interfaces do not reflect underlying
   hardware
4. xyz.openbmc_project.BMCPrebootContext (not yet fully rejected) //Arguably
   doesn't represent a hardware concept.
5. xyz.openbmc_project.UbootDeviceModel //There are ongoing efforts to enable
   dynamic device-tree modifications post-u-boot. The nodes this daemon reads
   are unlikely to be affected, but there's no guarantee of that.
6. xyz.openbmc_project.Platform //Some attributes like serial-number or MAC
   don't apply across a given platform
7. xyz.openbmc_project.Machine //I like this one, but sounds a little vague to
   me. I've suggested xyz.openbmc_project.MachineContext as an alternative.

## Impacts

The above-described process is only intended to scan selected paths -if they
exist- once during phosphor-u-boot-env-mgr startup.
...
the phosphor-u-boot-env-mgr subproject.

Since HPE will be relying on code in phosphor-u-boot-env-mgr for critical
functionality (HW identification), and since no other projects currently make
use of the phosphor-u-boot-env-mgr subproject, HPE will provide new maintainers
