...
1. xyz.openbmc_project.Decorator.\* -- Reason: "doesn't represent anything about
   the physical board" (-Ed Tanous)
2. ..FRUDevice Reason: The data is not coming from a FRU device

#### Proposed interfaces (rejected)

1. DeviceTree
2. uBootHWID
3. ..GXPFRU -- Reason: above interfaces do not reflect underlying hardware
4. ..BMCPrebootContext (not yet fully rejected) -- Reason: Arguably doesn't
   represent a hardware concept.
5. ..UbootDeviceModel -- Reason: There are ongoing efforts to enable dynamic
   device-tree modifications post-u-boot. The nodes this daemon reads are
   unlikely to be affected, but there's no guarantee of that.
6. ..Platform -- Reason: Some attributes like serial-number or MAC don't apply
   across a given platform
7. ..Machine -- Reason: I like this one, but sounds a little vague to me. I've
   suggested xyz.openbmc_project.MachineContext as an alternative.
