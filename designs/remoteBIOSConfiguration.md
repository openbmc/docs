...
#BIOS first boot
   ...
   | | Send the payload file |<---Req-/Res--->| Forward to the RBC daemon     |         |Del the Payload 1 if exist.  |    |-PayloadFlag   | ||
...
#BIOS DC cycle
   ...
   + |                       |             |  +-------------------------------+        +------------------------------+  | -FilePath       | ||
   | +-----------------------+             |  +-------------------------------+         +-----------------------------+  | -FileSize       | ||
   | |Set BIOS capability    |<---Req-/Res--> | Forward to the RBC daemon     |--dbus-->| Set the BIOSCapability      |  | -PayloadStatus  | ||
   | |                       |             |  |                               |         |  &inBandIntf                |  | -Payloadflag    | ||
   | |Set BIOS Pwd hash/Seed |<---Req-/Res--> | Forward to the RBC daemon     |--dbus-->| Set BIOSPwdHash data        |  | -PayloadChksum  | ||
   | |                       |             |  |                               |         | Set the IsBIOSInitDone      |  |                 | ||
   | |                       |             |  |                               |         |                             |  +-----------------+ ||
   | |Check factory settings |<---Req-/Res--> | Get the FactoryDefault prop   |<-dbus-- | Send FactoryDefaultSettings |                      ||
   | |Init the BIOS config   |             |  |                               |         |                             |    +---------------+ ||
   ...
   | |If Chksum mismatch     |<---Req-/Res--->| Forward new data to the RBC   |         |Delete the Payload 1.        |    |-PayloadFlag   | ||
...
#BIOS Dc cycle and BMC have new values
   ...
   | |If Chksum   match      |<---Req-/Res--->| Forward new data to the RBC   |<--dbus--|Send the Payload 1 info      |    |-PayloadFlag   | ||
...
PLDM capability 
ex:
 will send all bios tables and RBC will generate Payload 2 or will send Payload 2. 
PLDM daemon should send all BIOS tables or payload type 2 to the RBC daemon.
PLDM daemon should register for attribute changed signal and pull 
the Pending values tables from RBC daemon.
RBC daemon should generate the Attribute Registry format based on received
tables or PLDM should send payload type 2 (Attribute Registry format).
...
should set the RBC BIOS capability & collected data in Attribute Registry format
to the RBC daemon.
...
#BIOS first boot
   ...
   | |SetBIOSAttributeValue  |<---Req-/Res--> | Forward to the RBC Daemon     |--dbus-->| Create the Payload 5 Object |   |-Payloadchksum |  ||
...
1. Get the current BIOS settings attribute name and value pair - GET command:
 "https://<BMC IP address>/redfish/v1/Systems/system/Bios"
2. Get Bios Attribute Registry - GET command:
"https://<BMC IP address>/redfish/v1/Systems/system/AttributeRegistry"
3. Change BIOS password-POST command:
"https://<BMC IP address>/redfish/v1/Systems/system/Bios/Actions/Bios.ChangePassword"
4. Reset the BIOS settings - POST command:
 "https://<BMC IP address>/redfish/v1/Systems/system/Bios/Actions/Bios.ResetBios"
5. Update new Bios settings - PATCH command:
"https://<BMC IP address>/redfish/v1/Systems/system/Bios/Settings"
6. Get the new pending value list - GET command:
...
```
   ...
   | |                       |             |  +-----------------------+       +---------------------------+ ||
   | +-----------------------+             |  +-----------------------+       +---------------------------+ ||
   | |Get all BIOS Attributes|<---Req-/Res--> | Call RBC dbus Method  |-dbus->| getAllAttributes()        | ||
   | |                       |             |  |                       |       |                           | ||
   | |Get BIOS Attribute Reg |<---Req-/Res--> | Call RBC dbus Method  |-dbus->| getBiosAttributeRegistry()| ||
   | |                       |             |  |                       |       |                           | ||
   | |Change BIOS Password   |<---Req-/Res--> | Call RBC dbus Method  |<dbus- | changeBiosPassword()      | ||
   | |                       |             |  |                       |       |                           | ||
   | |To default settings    |<---Req-/Res--> | Call RBC dbus Method  |<dbus- | resetBios()               | ||
   | |                       |             |  |                       |       |                           | ||
   | |Get/update BIOS setting|<---Req-/Res--->| Call RBC dbus Method  |-dbus->| get/update BiosSetting()  | ||
   | |                       |             |  |                       |       |                           | ||
   | |Get attribute by name  |<---Req-/Res--->| Call RBC dbus Method  |-dbus->| getAttributebyName()      | ||
   | |                       |             |  |                       |       |                           | ||
   | |Set attribute by name  |<---Req-/Res--->| Call RBC dbus Method  |-dbus->| setAttributebyName()      | ||
   | |                       |             |  |                       |       |                           | ||
   | |Get attribute          |<---Req-/Res--->| Call RBC dbus Method  |-dbus->| getAttributeProperty()    | ||
   | |         property      |             |  |                       |       |                           | ||
   | +-----------------------+             |  +-----------------------+       +---------------------------+ ||
   |                                       +---------------------------------------------------------------+||
   +---------------------------------------+-----------------------------------------------------------------+
