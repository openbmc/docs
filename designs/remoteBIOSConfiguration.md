...
|       +---------MCTP----------------------->| into native-dbus format and    |                                 |
|                                             | send to the RBC                |                                 |
|                                             +--------------------------------+                                 |
+----------------------------------------------------------------------------------------------------------------+
```
...
#BIOS first boot
   ...
   | | Send the payload file |<---Req-/Res--->| Convert into dbus format      |                                       | ||
...
#BIOS reset
   ...
   | |then send the payload  |             |  | into dbus format and send to  |         |                             | ||
...
#BIOS reset and BMC have new values
   ...
   | |BIOS.                  |             |  | into native to dbus format    |         |                             | ||
   | |Reset the system.      |             |  |                               |         |                             | ||
   | +-----------------------+             |  +-------------------------------+         +-----------------------------+ ||
   |                                       +----------------------------------------------------------------------------+|
   +---------------------------------------+-----------------------------------------------------------------------------+
...
get attribute Dbus call or Wait for Pending attributes value signal and pull
...
Collected data in native to dbus format to the RBC daemon.
...
#BIOS first boot
   ...
   | |Based on value.        |             |  | native to dbus format         |--dbus-->| Attributes data             |||
   | |via Set BIOS table     |             |  |  by SetAllAttributes()        |         |                             |||
   | |                       |             |  |                               |         |                             |||
   | | Continue the BIOS boot|             |  |                               |         |                             |||
   | +-----------------------+             |  +-------------------------------+         +-----------------------------+||
   |                                       +---------------------------------------------------------------------------+|
...
#BIOS reset
   ...
   | |                       |             |  | native to dbus format by      |--dbus-->| Attributes data             |||
...
|  | Send PLDM data via MCTP  (or)    |  |                    | |  native to dbus format.            | |
...
```
   ...
   | |         list          |             |  |                       |       |                           | ||
   | |Set Pending attributes |<---Req-/Res--->| Call RBC dbus Method  |<-dbus-| SetPendingAttributes()    | ||
   | |         list          |             |  |                       |       |                           | ||
   | |                       |             |  |                       |       | SetAllAttributes()        | ||
...
RBC daemon should maintain attributes data in native-dbus format.
