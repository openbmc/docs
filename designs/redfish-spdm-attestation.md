...
device ID info to the dameon, e.g., configuration file, command line
parameters.

For PCIe DOE devices, SPDM daemon can enumerage all PCI devices under sysfs
and find out all BDFs with matching `VendorId:DeviceId`. SPDM daemon also
needs to query `InventoryManager` to get all PCI device inventory paths. By
querying `InventoryManager` managed objects and checking object interface 
...
we can find all PCI device inventory paths with matching `VendorId:DeviceId`.
So far, there is no universal way to map a given device's BDF to its inventory
...
daemon in dbus-sensors to handle the configuration. SPDM daemon will monitor
`mctpd` for `InterfacesAdded` signals providing the
`xyz.openbmc_project.MCTP.Endpoint` interface, which exposes the message types
supported by the endpoint in the `SupportedMessageTypes` member. SPDM daemon
set up a connection with the SPDM-capable endpoints to get certificates and
