...
For SPDM-over-TCP, SPDM daemon can query [phosphor-networkd][networkd-source]
for information about TCP endpoints, including the neighbor ip address and
create a connection either by using the reach out model (or) reach down model as
stated in the [SPDM over TCP specification][spdm-tcp-spec]

[networkd-source]: https://github.com/openbmc/phosphor-networkd
[spdm-tcp-spec]:
