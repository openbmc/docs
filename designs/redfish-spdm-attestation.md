...
PCIe-DOE or TCP setting up the transport layer connection could be easy. In this
design, we only consider SPDM over standard MCTP,TCP & PCIe-DOE connection.
...
For SPDM-over-TCP, SPDM daemon can query the phosphor-networkd for information
about TCP endpoints, including the neighbor ip address and create a connection
...
2. For each endpoint, which could be MCTP or PCIe-DOE or TCP, SPDM daemon query
...
- Make sure the API's are written in a scalable way that the framework could
  work with all types of transport mechanisms.
