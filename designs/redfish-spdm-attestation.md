...
As we know, BMCWeb provides SPDM Attestation support over Redfish. We will add
routes in the BMCWeb for ComponentIntegrity and TrustedComponent to support it.
But BMCWeb collects the information from D-Bus. The SPDM Attestation D-Bus
Daemon does the actual work.

SPDM protocol needs to be binded to a transport layer protocol, which transmits
...
PCI-VDM, SMBus/I2C, and so on. If the transport layer is using standard MCTP or
