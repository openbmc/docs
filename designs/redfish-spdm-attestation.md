...
`port` details from the `SatelliteController` interface and create a connection
either by using the reach out model (or) reach down model as stated in the [SPDM
over TCP specification][spdm-tcp-spec]

[entity-source]:
  https://github.com/openbmc/entity-manager/blob/master/schemas/satellite_controller.json
[spdm-tcp-spec]:
...
`xyz.openbmc_project.Configuration.SatelliteController` interface, which exposes
