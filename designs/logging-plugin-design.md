...
- Support producer-initiated plugins (explicitly requested by the log creator).
- Support framework-initiated plugins (automatically requested by framework
  policy or metadata).
...
Plugins may expose additional interfaces on the associated log entry object to
provide access to plugin-owned artifacts and metadata. These interfaces may
provide references to artifacts, direct access to artifact content, or both.
...
The plugin owns the implementation and lifecycle of any plugin-specific
interfaces associated with the log entry.
