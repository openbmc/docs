...
`pldm` for placing raw files into a specific directory (`/var/cper`) for which
there is no additional support in OpenBMC for consuming in any further way.
...
- Redfish representations must have a way to collect raw CPER binary data in
  addition to parsed CPER data. See the "CPER" property and the requirements on
  "AdditionalDataURI".
...
Extensions][event-extensions-design]. Any existing event, which are already
...
as CPER content to an existing event. There is proposed two new dbus interfaces:

- [`xyz.openbmc_project.Logging.Extension.CPERProcessed`][cperprocessed-gerrit]
- `xyz.openbmc_project.Logging.Extension.CPERRaw`

TODO: add references to Gerrit link for `CPERRaw`.
...
are already known to be compliant with Redfish specificaitions. The only parts
...
additional processing is required on the data by `bmcweb` (ie. the data is in
"Redfish native" format on dbus).

#### Exposing Raw CPER

There is one hang-up between the existing implementation and the raw CPER data.
The Redfish specification requires that if we use a `DiagnosticDataType` as
`CPER` or `CPERSection` then `AdditionalDataURI` should contain a pointer to the
raw CPER data. In the `phosphor-logging` implementation, this
`AdditionalDataURI` is already used for the raw BMC event from the
`phosphor-logging` file system and we should not take away this functionality.
Instead, we will keep the `DiagnosticDataType` as `OEM` and the
`AdditionalDataURI` will continue to be the raw `phosphor-logging` entry.

Instead, we propose a new `LogService` exclusively to hold the raw CPER data but
not the processed CPER data. If a client needs the raw data, they can follow an
odata link between the `LogEntry` created by the `phosphor-logging` data and the
`LogEntry` in this new `LogService` representing the raw CPER data. Only in this
place will the `DiagnosticDataType` be `CPER` or `CPERSection` and the
`AdditionalDataURI` contain the raw CPER binary.
...
Once prior art proposal was to create an entirely separate repository for CPER
...
- There is currently no support for push-events in that implementation and would
  entail an entirely distinct implementation in `bmcweb`.
...
- Principle contributors will run a full end-to-end test after integration.

- Principle contributors may add `openbmc-test-automation` tests for mocked CPER
  content.
