...
- The way HMC locks is little different than how a normal redfish resource management
  work,all the HMC resources are hypervisor resources and the BMC does not have
  any knowledge regarding them.

## Example of Multi level structure for Locking

Below is an example resource hierarchy and this resource hierarchy is actually
maintained with-in the HMC and BMC does not even know about what resources are
present in the hierarchy when a lock request comes from HMC.But it merely checks
for the conflicts across resources between the incoming request & the existing ones.

It is also to be noted that:
- The HMC is responsible for creating locks with the level(hierarchy) information
  embedded in it, that information is contained in the ResourceID & Segment Flags
  which comes in as a part of lock request.While BMC does not understand anything
  about the resource that it is actually locking, but it merely checks for conflicts
  between two lock requests.

- These parent-child hierarchies are neither Redfish hierarchies nor DBus
  object paths.These are completely owned & maintained by the HMC and is not at
  all bothered by BMC.
...
1. The HMC would construct a string in the form of a PATH which would indicate the
   ...
   to form a tree structure so that it could find the conflicts against the new lock
   request.
2. A procedure in which we maintain a one-to-one mapping between the HMC resources
   ...
   to the give problem.
3. Another thought point was that, BMC could read the MRW to get the knowledge
