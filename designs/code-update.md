...
pldmd. The current approach https://gerrit.openbmc.org/c/openbmc/pldm/+/74774
starts parallel update for two components implemented by the same PLDM FD which
will cause FW update to fail for one of the components and will break the
requirement #13. To meet this requirement pldmd needs to implement the 
xyz.openbmc_project.Software.Update D-Bus interface at the application level
and not for individual FW inventory, this will allow pldmd to orchestrate the
update of PLDM devices according to the specification. There are two approaches
described here, one orchestrated by pldmd and other orchestrated by bmcweb, the
difference is if Non-PLDM updaters are initiated by pldmd or bmcweb.

#### Orchestration by pldmd

bmcweb to forward the multi-part image to the pldmd daemon along with the update
parameters like Targets, ForceUpdate. pldmd will parse the PLDM package and
initiate the FW update for PLDM devices based on the matching algorithm as defined
in the PLDM T5 specification. pldmd will map Targets to corresponding MCTP endpoints
and PLDM component IDs. pldmd aggregates status across PLDM devices and non-PLDM
code updaters.
...
2. Non-PLDM code updaters:
   ...
   - pldmd invokes the non-PLDM code updaters based on OEM D-Bus interfaces.
     This can be made common D-Bus interface, if this option can be shared
...
parsed by the code updaters. bmcweb will aggregate the status and progress across all the code updaters to a single RF
task.
