...
The PLDM for
[Firmware Update Specification](https://www.dmtf.org/sites/default/files/standards/documents/DSP0267_1.2.0.pdf)
...
modeling device access details. For instance, the following example demonstrates
how this can be achieved for an i2c CPLD device. These D-Bus objects can then be
consumed by the CodeUpdater service to manage updates for the relevant firmware
entities.
...
"Exposes": [
  {
    ...
    "Type": "CPLD",
    "VendorId": "XXXXX",
    "Compatible": "com.meta.Hardware.CPLD.LCMX02_2000HC",
    "Bus": "XX",
    "Address": "0xXX",
  },
  ...
...
- `Type`: This field is used by the CodeUpdater service to determine which
  firmware configurations it should process.
- `VendorId`: This field corresponds to the `IANA Enterprise ID` descriptor in
  PLDM package header.
