...
firmware updates for non-PLDM devices as well. For such devices, at a minimum,
the following descriptors can be processed and validated by CodeUpdaters.
...
- For a PLDM device, the `Component` field corresponds to the
  `ASCII Model Number` of the downstream device.
