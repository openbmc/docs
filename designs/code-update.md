...
code update manager daemon. The initial code update manager will support PLDM as
the multi part image format and will use libpldm library functions to parse the
PLDM package. bmcweb will invoke the code-update-manager image if no Targets are
provided. code-update-manager will use the entity-manager configuration to map
the components in the PLDM package to the individual code updaters. Since the
package format is PLDM, code-update manager will also forward the PLDM package
to the PLDM UA which will match the package with the PLDM T5 endpoints as
defined in the PLDM T5 specification.
