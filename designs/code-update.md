...
4. Unprivileged daemons with sole access to DBus should be able to accept and
...
11. Able to restrict critical system actions, such as reboot, while the code
    update is in flight.
...
[Sequence Diagram](https://github.com/openbmc/docs/blob/387e6332fd3eb9337b083942e844abcce4a01177/designs/code-update.md)
...
BMCW ->> CU: StartUpdate(ImageFd, UpdateSettings)
...
For BMCWeb to forward an update request to the appropriate
\<deviceX>CodeUpdater, a association needs to be maintained from
xyz.openbmc_project.Software.Version to xyz.openbmc_project.Inventory.Item named
...
[Issue# 3](#problem-description) and [Requirement# 3](#requirements).
...
- Extend openbmc_project.Software.Update.StartUpdate method parameters to
  include SystemInventoryItem path, so \<deviceX>CodeUpdater can map the update
  operation to the appropriate device.
...
`ActivationBlocksTransitions` interface will be created on the specific DBus
...
repository. For example, all devices using PMBus can have a common repository.

### Who will be the initial maintainer(s) of this repository?

Meta will propose repositories for following devices and `Jagpal Singh Gill`
will be the maintainer for them.
