...
- Client server doesn't have to maintain a Dbus matcher
  ([Issue](https://github.com/openbmc/bmcweb/issues/202)).
- Easier implementation in client server as no asynchronous handlers would be
  required.

#### Cons

- Client server would still need maintain some info so it can map client request
  to Dbus path for /xyz/openbmc_project/Software/<deviceX> for calling
  getActivationStatus and getActivationProgress. Otherwise, client server would
  need to perform two Dbus calls for every such request - first to get DBus path
  and second for method call. With proposed solution, the matcher updates will
  happen asynchronously and client server will return the result locally.
...
- Currently, activation and progress interfaces are being used in
  [lot of client servers](#organizational). In future, harmonizing the flow to
  single one will involve changing the push to pull model in all those places.
  With the current proposal, the only change will be in update invocation flow.

## Impacts

The introduction of new DBus API will create two different invocation flows from
client server. Client servers (BMCWeb, IPMI, etc) can initially support both the
code stacks. As new API gets soaked into the system and gets wider acceptance,
client servers can be changed to completely use the new API stack.
...
The end to end integration testing involving client servers (for example BMCWeb)
will be covered using openbmc-test-automation.
