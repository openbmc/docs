...
running software version for the associated inventory item. The `run_by` would
be the corresponding reverse association.

`acting` : An `acting` association from xyz.openbmc_project.Inventory.Item to
xyz.openbmc_project.Software.Version represents the activated (but not yet run)
software version for the associated inventory item. There could be more than one
active versions for an inventory item, for example, in case of A/B redundancy
models there are 2 associated flash-banks and
...
The `acted_by` would be the corresponding reverse association.
