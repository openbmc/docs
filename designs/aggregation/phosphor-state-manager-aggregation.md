...
corresponding PSM instances. The PSM applications on the sleds will continue to
create objects with instance 0 and will only have knowledge of, and be
responsible for the sled it is running in.
...
- Aggregator is subscribed to each sleds CurrentHostState and BootProgress
  ...
  - The d-bus properties in `ComposedSystemHost0` are what are utilized by the
    aggregator for returning composed system state to Redfish queries
...
- The Aggregator will execute recovery (either report error and keep composed
  system in Quiesced state or remove the bad sled from the composed system and
  reboot)
