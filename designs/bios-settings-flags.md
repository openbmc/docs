...
a risk that a system restart occurring hours or days later could cause an unexpected
boot sequence if the boot options remain configured without proper oversight or
reconfiguration. The BMC must automatically clear the 'boot flags valid bit' if a
system restart is not initiated by a Chassis Control command within 60 seconds
±10% of the valid flag being set [IPMI v2.0 Specification][1](section 28.12).
The current implementation is missing the timer functionality required for timeout
configuration and detection. While ipmitool allows users to set timeout values, the
BMC lacks the underlying support to actually activate, track, or enforce these timeout mechanisms.
The boot detection can also be used by other application that also need to be notify in case
of host booting.
...
The current implementation enables timeout configuration via ipmitool
but does not implement the timer mechanism required to enforce the timeout.
In the new design, the bios-settings-mgr will manage and control this
timer functionality.
...
- Set to `false` by BMC if 60-second timer expires and we did not get BIOS boot
  indication (prevents UEFI from using configuration). The timer will be canceled
  if BootProgressLastUpdate has changed. The BootProgressLastUpdate flag is implemented
  the phosphor-state-manager [Docs design][2].
...
**Redfish:**
DNTF system computer schema [Computer System schema][3]
...
**D-Bus Path:**
A new D-bus interface to control the timeout configuration:
...
An alternative approach would be to place the timeout mechanism in the phosphor-ipmi-host
...
[2]:
   ...
   "Design of BootProgressLastUpdate"
[3]:
