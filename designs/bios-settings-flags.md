...
sequenceDiagram
    ...
    BIOS->>"Update BootProgress Service": Boot signal
    Note over BIOS, "Update BootProgress Service": BIOS sends boot notification signal

    "Update BootProgress Service"->>"Update BootProgress Service": Update BootProgress attribute to "PrimaryProcInit"
    Note over "Update BootProgress Service": Watch BootProgress attribute

    "Update BootProgress Service"->>"BIOS-settings Manager": Attribute change notification
    Note over "BIOS-settings Manager": "Update BootProgress Service" service notifies when attribute changes
...
1. **BIOS** When Bios is booting the BootProgress is updated.
...
[Update BootProgress Service] - Service that monitors the BIOS booting and changes
