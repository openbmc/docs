...
- Set to `false` by BMC if 60-second timer expires and we did not get BIOS boot
  ...
  canceled if BootProgressLastUpdate has changed. The BootProgressLastUpdate
...
boots within 60 seconds and the timer is canceled, for one-time settings, the BIOS
will set the enable flag back to false and apply the one-time boot configuration.
