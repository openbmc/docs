...
- Set to `false` by BMC if 60-second timer expires and we did not get BIOS boot
  ...
  canceled if BootProgress property has change to PrimaryProcInit stage. The BootProgress
  flag is implemented the phosphor-state-manager [Docs design][2].
...
sequenceDiagram
    ...
    "Platform Specific Service"->>"Platform Specific Service": Update BootProgress attribute to "PrimaryProcInit"
    Note over "Platform Specific Service": Watch BootProgress attribute
...
2. **Platform Specific Service** receives the signal and updates the
   `BootProgress` property to "PrimaryProcInit".
3. **BIOS-settings Manager** watches the `BootProgress` attribute for
   changes.
4. **BIOS-settings Manager** when `BootProgress` changes to "PrimaryProcInit", cancels the
...
flag to false,causing the BIOS to ignore the settings when it boots. If the BIOS
boots within 60 seconds and the timer is canceled, for one-time settings, the
BIOS will set the enable flag back to false and apply the one-time boot
configuration.
...
the BootProgress attribute to "PrimaryProcInit" when BIOS start booting.
