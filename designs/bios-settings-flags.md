...
±10% of the valid flag being set [IPMI v2.0 Specification][1]. The current implementation lacks proper BIOS
...
requirements for boot configuration management in enterprise systems. The
[IPMI v2.0 Specification][1] mandates automatic timeout mechanisms for boot flags to prevent
...
An alternative approach would be to place the timeout mechanism in the ipmi-host repository, where the
handlers for setting the attributes are also located. However, since some of these attributes can also
be modified by Redfish, it is preferred that the timeout logic be implemented in a location that is
...
[Platform Specific Service] - Service that monitors the BIOS booting and
changes the BootProgressLastUpdate attribute accordingly.
...
Testing can be performed directly on the BMC by rebooting the system and verifying
the attributes through D-Bus, IPMI, and Redfish interfaces.
...
[1]: https://www.intel.com/content/dam/www/public/us/en/documents/product-briefs/ipmi-second-gen-interface-spec-v2-rev1-1.pdf "IPMI v2.0 Specification"
