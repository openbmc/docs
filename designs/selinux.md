...
As a prerequisite, a system integrator will need to obtain meta-selinux
from the yocto project git repo, and place it under their local openbmc/
top level directory:
...
Finally, system integrators will need to apply changes to their target-specific
...
* Any target-dependent policies should be stored in the target
  recipe directory. For example, policies that are specific to the evb-ast2600
  target should be placed under
  meta-evb/meta-evb-aspeed/meta-evb-ast2600-selinux.

  System integrators, working with their security team, will probably need
  to fine-tune the policy for their specific use cases, before choosing to
...
Since D-Bus has built-in support for SELinux, a future design could
