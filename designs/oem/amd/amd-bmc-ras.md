...
The Common Platform Error Record (CPER),as outlined in
[the UEFI specification](https://uefi.org/specs/UEFI/2.10/), contains vendor
...
Crashdump records saved in the non-volatile location can be retrieved via the
redfish interface.
...
BMC RAS application is started by the systemd service com.amd.RAS.service

The proposal focuses on re-using the existing
[phosphor-debug-collector](https://github.com/openbmc/phosphor-debug-collector)
to host the CPER file informations to the dbus interface once the Crashdump
files are created by the amd-ras application. A new property "FaultLogType" will
be added to the interface `xyz.openbmc_project.Dump.Create`. The RAS application
invokes the `CreateDump()` API which is associated with the existing d-bus
object path (`xyz/openbmc_project/dump/faultlog`). The FaultLogType and CPER
file path informations is passed to the `CreateDump()` API to create new
faultlog entry d-bus object path.
...
`/redfish/v1/Systems/system/LogServices/FaultLog` is used to list/delete/
download the files. The `DiagnosticDataType` is populated as "CPER" as defined
in the `LogEntry` redfish schema.

A D-Bus interface `com.amd.RAS.Configuration` is also maintained which has the
config file info.
...
A Redfish interface is required which should allow the user to modify the
properties of RAS config file. The design document will be updated with the
...
more appropriate to store them in
`/var/lib/phosphor-debug-collector/faultlog/ <dump id>/CPER_FILE`.
...
interface `xyz.openbmc_project.RAS.Configuration` will store the user configured
values. An additional new property "FaultLogType" will be added to the interface
`xyz.openbmc_project.Dump.Create`.

Minimal changes required in `phosphor-debug-collector` to parse the CPER file
path in FaultLog CreateDump() API.
...
Fatal errors can be simulated artificially using the error injection mechanism
and the error conditions will be logged by the bmc ras module. An analyzer tool
can be used to decode the data available in the CPER record to get a detailed
