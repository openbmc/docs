...
- **Entity-Manager probes are extended to handle reading data from arbitrary
  ...
  - **Cons:**
  - Potential to adversly affect existing probe behavior; Wouldn't be isolated
    from general probe processes like when using a seperate daemon for
    specific-channel interactions
  - May encourage undesired device-tree explotation

- **A reserved-memory node in device-tree is used for transport of
  'serial-number' and 'model' ID data**
  - **Pros:**
    - Fewer daemons, since HPE is already looking at using reserved-memory for
      HPE-specific HW ID details like board revision IDs (to be discussed in a
      separate OEM/HPE/ design doc)
  - Reserved-memory is commonly used for passing data from firmware
  - **Cons:**
    - Has no well-known handles for common properties like device-tree/model or
      serial-number like device-tree does; makes more sense for keeping
      arbitrary data instead of 'well-known' properties
  - The critical identifier HPE wants to rely on for platform identification
  ...
  - Since device-tree/model is well described in device-tree documentation,
...
- **All HW ID data is written to file paths in /proc/device-tree/chosen/**
  ...
  - **Pros:**
    ...
    - Is not being used by other BMC platforms, so easy to enable/disable
  ...
  - **Cons:**
    - Not a 'usual' location for passing this kind HW-tied data. Unlikely to
...
- **HW ID data is published to u-Boot enviornmental variables, then transfered
  ...
  - **Cons:**
    ...
    - Saves a negligible amount of cycles at Entity-Manager startup for
