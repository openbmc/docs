...
management unit.
[Bios-settings-manager](https://github.com/openbmc/bios-settings-mgr) provides
ability for the user to view and modify BIOS setup configuration parameters
remotely via a BMC at any Host state. This design involves implementing the
Redfish Host Interface in OpenBMC to support remote BIOS configuration.

## Requirements

This feature aims at supporting standardized Redfish-based communication between
the BIOS and the redfish server running in the BMC providing system manufactures
to easily integrate the feature to existing management controller technology.
...
4. Creation and management of BootStrap account created for Redfish
   communication.

This design includes:

- New D-Bus interface for Redfish resources `HostInterface`.
- BMCWeb changes for supporting
  [HostInterface](https://redfish.dmtf.org/schemas/v1/HostInterface.v1_3_3.yaml)
  resources.
...
4. bmcweb interacts with `host-interface-daemon` to implement
   [HostInterface](https://redfish.dmtf.org/schemas/v1/HostInterface.v1_3_3.yaml)
   schema.
5. BIOS updates the `BIOS Attributes`, `BootOption`, `BootOrder` and
   `SecureBoot` information through Redfish(bmcweb), which is then converted
   into the native DBus format and passed to `bios-settings-manager`.
6. Once these properties are available, a remote user can modify them as
   described in
   the[bios-settings-manager](https://github.com/openbmc/bios-settings-mgr).
...
1. The BIOS first retrieves the BMC USB interface information, such as VendorId
   and ProductId, using the "Get USB Description" IPMI OEM command.
2. The BIOS then reads the serial number of the USB interface with the "Get
   Virtual USB Serial Number" IPMI OEM command.
3. The BIOS fetches the Redfish Hostname using the "Get Redfish Hostname" IPMI
   OEM command.
4. The BIOS obtains the channel number for the BMC network interface, which is
   configured for the Host Interface, through the "Get IPMI Channel Number" OEM
   command. Using this channel number, the BIOS then retrieves the network
   configuration details with the standard IPMI command, "Get LAN Configuration
   Parameters."
5. Once the handshake is complete and basic communication between the BIOS and
   BMC is established, the BIOS sends the "Get Bootstrap Account Credentials"
   IPMI command to acquire the auto-generated credentials.
6. After the Bootstrap Account is created, the user information is updated in
   the `phosphor-user-manager`. The user is added under `redfish-hiface` group
   and is not shown under `ipmi user list`.
7. The Bootstrap Account related properties associated with the
   `Host-Interface-daemon` are updated by `ipmid`.
8. The BIOS uses the Bootstrap Account to interact with the BMC via Redfish and
   update BIOS properties.
9. Once the BIOS properties are available, a remote user can modify them as
   described in the
   [bios-settings-manager](https://github.com/openbmc/bios-settings-mgr).
...
2. BMC generates username and password to be used for further communication and
   ...
   well as Redfish `AccountService` schema. The HI users created by BMC will be
   added under "redfish-hostiface" group to disable it from showing under IPMI
   user list. The username and password shall
...
- Can be used to manage BIOS settings such as Boot Order, Boot Option, Secure
  Boot, and other system parameters.
...
- Does not natively support BIOS configuration tasks like Boot Order, Boot
  Option or Secure Boot, which limits its use for managing BIOS-specific
  settings.
