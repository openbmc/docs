...
- The action on failure to log an audit event is configurable and includes the
  ...
  drop audit events.
- Start and stop of audit service is logged.
...
The Linux auditd daemon logs events when the daemon starts and stops.
Additionally it logs errors to log or rotation of the logs to the system log.
