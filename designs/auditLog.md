...
This design provides a mechanism for preserving a history of modifications and
provides a mechanism to make it available to a system administrator. Details are
included for using this mechanism for recording Redfish API modification events
through bmcweb. The mechanism provided could be used by other OpenBMC subsystems
to record events coming through their interfaces. Further updates of this design
in the future can document the details of using this mechanism with other entry
points.

The bmcweb entry points are prioritized as the most common use case. No further
updates are planned for the IPMI specification per the
...
- It provides much flexibility and many options for configuration including
  handling on error to write audit event.
...
- Stress logging of audit events: On a heavily loaded BMC, continuously perform
  various events which should produce audit records, and validate all events are
  audited within audit configuration parameters. Stress can be from various BMC
  operations such as firmware upload (without rebooting the BMC), reading logs,
  or a program running which chews up spare CPU cycles, etc. Run for an hour at
  minimum.
