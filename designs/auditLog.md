...
Linux Kernel Audit Subystem provides many benefits:
...
obtaining authentication (currently _pamAuthenticateUser()_) rather than in
_completeRequest()_.

A benefit of enabling and using the Linux auditd subsystem is that the
configuration can be extended to log other events. For example details for
auditing ssh login sessions are not included as part of this design. However the
Linux auditd subsystem can be configured to log ssh sessions. This answer in
StackExchange on the
...
an in-memory only solution. Addtionally using any of the existing logging tools
...
and implementation of the new logger. The update seems to have been abandoned
mainly due to inaction of addressing the issues. See
...
addition of the IP address and user into the bmcweb requests in order to log
this information. It would also require the addition of logging by the PAM
