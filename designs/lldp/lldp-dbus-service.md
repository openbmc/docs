...
2. lldpcli is an online tool that can be used to control the daemon during runtime.
3. liblldpctl.so library provides a stable API for controlling lldpd.
...
The service controls the lldpd daemon with liblldpctl tool and lldpd configuration files.
...
The service will only interact with the lldpd daemon when when required by busctrl commands.
...
The design proposes a new D-BUS service, so is no impact on other D-BUS services.

There will be a minor additional memory overhead.

The The D-BUS service controls LLDP only when called by busctrl commands. So the runtime overhead is also small.
