...
The service will only interact with the lldpd daemon when when required by
busctrl commands.
...
This file configures the LLDP interface. The service reads this file to obtain
the <if_name> value.
...
The The D-BUS service will update the properties only when the properties are
