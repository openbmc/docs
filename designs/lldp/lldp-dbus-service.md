...
- `LLDPTransmit` properties map to `xyz.openbmc_project.Network.LLDP.ROTLVs` and
   `xyz.openbmc_project.Network.LLDP.RWTLVs` via transmit D-BUS objects
- `LLDPReceive` properties map to `xyz.openbmc_project.Network.LLDP.ROTLVs` and
   `xyz.openbmc_project.Network.LLDP.RWTLVs` via receive D-BUS objects
...
This busctl command will get the property SystemDescription, and it should be
"bmc":
