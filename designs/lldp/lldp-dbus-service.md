...
This service is indented for users that have a BMC connected to an L2 network
with LLDP enabled, and require to monitor and manage LLDP via Redfish.
...
Since there can be several network interfaces configured for LLDP,each D-BUS
object will manage a specific network interface.

The interfaces were designed for compatibility with Redfish LLDP DMTF properties
in:
<https://github.com/openbmc/bmcweb/blob/master/redfish-core/schema/dmtf/csdl/Port_v1.xml>.
...
1. `xyz.openbmc_project.Network.LLDP.Settings`
   - Control over LLDP functionality for a network interface. It controls both
     Tx and Rx at the same time.
   - Properties:
     - `EnableLLDP` (bool): Enable or disable LLDP functionality

2. `xyz.openbmc_project.Network.LLDP.ROTLVs`
   - This interface provides access to all read only LLDP TLV information.
...
3. `xyz.openbmc_project.Network.LLDP.RWTLVs`
   - This interface provides access to all read write LLDP TLV information.
   - It is used by the transmit and receive D-BUS objects.
   - For the receive D-BUS object, these properties are read only.
   - Properties:
     - `ChassisId` (string)
     - `ChassisIdSubtype` (string)
     - `PortId` (string)
     - `PortIdSubtype` (uint16)
     - `SystemDescription` (array[uint8])
...
2. phosphor-D-BUS-interfaces/yaml/xyz/openbmc_project/Network/LLDP/ROTLVs.interface.yaml
3. phosphor-D-BUS-interfaces/yaml/xyz/openbmc_project/Network/LLDP/RWTLVs.interface.yaml
...
The D-BUS service creates and deletes Neighbor D-BUS objects based on syncing
with the lldpd daemon neighbor list. lldpd daemon neighbor list is updated when
a new neighbor send a LLDP packet, or an existing neighbor did not send LLDP
packets for a set amount of time.
...
- `LLDPTransmit` properties map to `xyz.openbmc_project.Network.LLDP.ROTLVs`
   and `xyz.openbmc_project.Network.LLDP.RWTLVs` via transmit D-BUS objects
- `LLDPReceive` properties map to `xyz.openbmc_project.Network.LLDP.ROTLVs`
   and `xyz.openbmc_project.Network.LLDP.RWTLVs` via receive D-BUS objects
...
This busctl command will set the property SystemDescription as "bmc":
...
This busctl command will get the property SystemDescription, and it should be "bmc":

busctl get-property xyz.openbmc_project.LLDP
/xyz/openbmc_project/network/lldp/eth0/transmit
xyz.openbmc_project.Network.LLDP.TLVs SystemDescription

This shell command should also receive the SystemDescription information:
