...
LLDP is preferred over other discovery protocols like Avahi because it operates at Layer 2  making it independent of IP configuration and routing. Additionally, LLDP is specifically designed for network equipment discovery, provides detailed hardware and topology information, and it has DMTF Redfish compliance, making it the appropriate choice for BMC network management.
...
specification in `https://github.com/openbmc/bmcweb/blob/master/redfish-core/schema/dmtf/csdl/Port_v1.xml`.
...
It will be added to existing repository phosphor-networkd.
...
3. `/xyz/openbmc_project/network/lldp/<if_name>/settings`
...
3. Enable/Disable LLDP
   ...
   - Uses lldpd configuration files to maintain the enable status between BMC resets.
...
The Redfish specification in `https://github.com/openbmc/bmcweb/blob/master/redfish-core/schema/dmtf/csdl/Port_v1.xml`
...
- `LLDPTransmit` properties map to `xyz.openbmc_project.Network.LLDP.TLVs` (via transmit D-BUS objects)
- `LLDPReceive` properties map to `xyz.openbmc_project.Network.LLDP.TLVs` (via receive D-BUS objects)
...
directionality to transmit or receive, or saving the lldp state
between BMC resets buy updating lldp configuration files.
