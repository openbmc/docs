...
`LLDPTransmit` and `LLDPReceive` have the same properties so a single D-BUS
interface `xyz.openbmc_project.Network.LLDP.TLVs` is used both for transmit and
receive.

`LLDPEnabled` controls lldp functionality for network interface turning it on or
off. It controls both Tx and Rx at the same time.
