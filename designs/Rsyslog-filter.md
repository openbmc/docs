...
server. This causes excessive outbound traffic, poor signal-to-noise ratio, and
potential exposure of low-value or sensitive logs.
...
Redfish for external API, D-Bus for internal contracts, and backend services for
...
{ "SyslogFilters": [ { "LogFacilities": ["Kern", "User"], "LowestSeverity":
"Warning" } ] }
...
- facilities[]: enum {kern, user, mail, daemon, auth, syslog, lpr, news, uucp,
  cron, authpriv, ftp}
...
- Rejected because it leaks backend-specific syntax and weakens API portability
