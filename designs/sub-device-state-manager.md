...
5. All sub-device types share a single `State.Device` interface for transition
   ...
   possibility (see Background) and is not ruled out here. Put differently, this
   is about adding a _type_; an instance whose sequence only recombines existing
...
group name, a required wait timeout, and, the first time a given group name is
used, the maximum number of concurrent holders it allows; every later
`AcquireGroup` naming that same group must agree with the maximum already set
for it, since the daemon enforces one limit per group name, not one per
reference. Omitting the maximum defaults it to one: the one real precedent this
design has checked (a board-wide drive-power lock) is exactly this mutual-
exclusion case, so a platform whose limit is genuinely higher than one states it
explicitly. A wait that exceeds its timeout is an ordinary operation failure,
handled the same fixed way as any other (see below), not a platform-defined
outcome. The boundary a group name represents beyond its numeric limit is
entirely a platform decision expressed in its own configuration: a group can
name a board-wide resource shared across instances, or a single instance's own
serialization, scoping it however the platform's own sequences reference it; the
daemon only enforces the limit for whatever grouping it is given. When and how
often a sequence explicitly acquires and releases a group within its own steps
is likewise the platform's choice; the one thing the daemon guarantees on its
own is that any group still held when a transition's or state-query's top-level
sequence ends, whether it succeeded, failed, or was aborted, is released at that
point, so a platform's configuration only chooses when it releases early, never
whether the daemon eventually gives a forgotten acquisition back.
...
sequence, and reports success immediately. This applies only to `On` and `Off`,
which are requests to reach a state; `Reboot`, `HardReboot` and `PowerCycle` are
requests to perform an action and always run their sequence regardless of the
device's current state, since asking to reboot a device that happens to already
be `On` is not the same request as asking for it to be `On`. When an instance
permits overlapping sequences this way, by not placing them under a common
group, each sequence updates `CurrentDeviceState` according to its own progress
and result as it runs, and the most recent update wins, whichever sequence
produced it; the daemon does not impose per-instance serialization beyond the
...
Transition targets, one per `Transition` value, each started by the daemon only
once that transition's own sequence completes successfully (see Sequence
Execution). A sequence that fails does not start the transition target; see
below for what the daemon does instead.
...
when the transition genuinely succeeded, not merely attempted. A platform
needing a companion action on success attaches its own service to this same
target via `.wants/` in its own meta-layer, exactly as it would for any other
target; no additional daemon mechanism is needed for that case.

A sequence that fails instead starts a fixed failure target, one per
`Transition` value, derived from the same type-table prefix and suffix set as
the targets above:

```text
obmc-drive-poweron-failed@.target       Started when the On sequence fails
obmc-drive-poweroff-failed@.target      Started when the Off sequence fails
obmc-drive-reboot-failed@.target        Started when the Reboot sequence fails
obmc-drive-hard-reboot-failed@.target   Started when the HardReboot sequence fails
obmc-drive-powercycle-failed@.target    Started when the PowerCycle sequence fails
```

A platform needing a recovery action on failure attaches its own service to the
matching failure target the same way, via `.wants/` in its own meta-layer,
mirroring the existing `chassis-poweron-failure@%i.service` pattern. These names
are entirely daemon-derived, from the same type-table prefix and fixed suffix
set as the success targets; entity-manager configuration cannot name an
arbitrary unit for the daemon to start on either success or failure. Naming an
arbitrary unit from entity-manager data would be an unaudited extension point a
platform could route real hardware operations through instead of the closed
operation vocabulary, which is exactly what this design otherwise declines to
allow (see Alternatives Considered); fixed, daemon-derived target names give a
platform the same `.wants/`-based attachment point without opening that door.
