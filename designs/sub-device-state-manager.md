...
Platforms provide a Board entity with `Inventory.Item.Drive`. All properties
defined in the PDI interface are included so that the D-Bus interface is fully
populated when entity-manager creates the object. `Protocol`, `Type`, and
`Resettable` are platform-known static values. `Capacity`, `EncryptionStatus`,
`LockedStatus`, and `PredictedMediaLifeLeftPercent` use PDI defaults and are
updated at runtime by monitoring daemons when the drive is present and
accessible.
...
When a `RequestedDriveTransition` of `On`, `Reboot`, or `Powercycle` is received
...
**Compiled-in hardware control via abstract base class.** Instead of systemd
targets as the extension point, platform-specific hardware operations (GPIO,
I2C, NVMe-MI) are compiled directly into the daemon binary. Each platform
implements an abstract base class in a per-platform sub-directory, and the
common code calls into it at build time or via runtime detection. This gives
full static analysis coverage and allows maintainers to refactor freely, at the
cost of coupling platform hardware code into the state manager source tree. A
similar pattern is used in `phosphor-bmc-code-mgmt` where common code calls into
device-specific update implementations.
