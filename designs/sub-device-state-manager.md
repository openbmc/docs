...
6. Per-type requirements such as host dependency are expressed through per-type
   mechanisms (systemd target dependencies for execution ordering, and optional
   slot metadata for semantic validation), not imposed on all device types by
   the framework.
...
Each per-type binary is long-lived and single-instance. Entity-manager discovery
creates per-device state objects at runtime within the single process, so there
is no systemd template unit per device instance. The first supported type is the
drive binary (`phosphor-drive-state-manager` running as
...
published by entity-manager. A slot configuration may declare an optional
`HostId` property identifying which host the slot belongs to. When a
`RequestedDriveTransition` of `On`, `Reboot`, or `Powercycle` is received, the
daemon reads `CurrentHostState` for the associated host; if the host is not in a
state that permits drives to be powered on, the transition is rejected with a
D-Bus error, which bmcweb surfaces as a Redfish 4xx response. Slots without a
`HostId` declaration receive no semantic check and behave as host-independent
devices.
...
`xyz.openbmc_project.Inventory.Item.Drive` as a top-level Board interface, and
to define a `HostId` property on slot configurations so platforms can declare
drive-to-host association for semantic validation.
...
- Which repositories are expected to be modified?
  ...
  - `entity-manager` (schema: allow `Inventory.Item.Drive`; add `HostId` slot
    property)
