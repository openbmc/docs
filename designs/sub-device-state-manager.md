...
PCIe card) to power control adds a table entry and the corresponding
entity-manager configuration; it requires no new interface, no new binary, and
no new systemd target units, since the transition-notification targets are
already generic across every type.
