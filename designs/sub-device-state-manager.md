...
transitions including new `On` and `Off` values. `State.Drive` retains
type-specific properties such as `CurrentDriveState` and gains a `paths:`
metadata block for the object path convention.
