...
3. Separate hardware-specific power control (GPIO toggling, I2C IO expander
   ...
   get/wait, a terminus command, sleep, a named concurrency limit, and a bounded
   retry around a sub-sequence); a platform supplies each instance's ordered
   sequence of operations directly in its entity-manager configuration, but
   cannot introduce a new kind of operation.
...
of operations specific to that instance, in a format the daemon defines. There
is no intermediate named template stored in phosphor-state-manager;
...
configuration supplies. Keeping each instance's sequence directly in
entity-manager, rather than introducing a named, reusable template that
entity-manager would only select by name, follows entity-manager's own stated
design: its README accepts duplicated configuration across per-model files as a
deliberate trade-off for having every supported device's full configuration
readable in one place, and states that entity manager itself does not
participate in managing any device, only publishes the data a reactor consumes.
A per-instance sequence is exactly that kind of data; phosphor-state-manager
remains the sole interpreter and executor, so control flow, error handling, and
event logging stay in the daemon regardless of how many instances'
configurations look alike (see Alternatives Considered).
...
conditional branching on device-specific data.
...
instance's own physical binding; there is no placeholder to resolve, because
there is no shared template being reused across instances.
...
sequence expresses this with `wait_gpio` against a real settle signal where one
exists, which resolves immediately if the device was already off long enough and
only waits when power was just removed; a device with no such signal falls back
to an unconditional `sleep` at the same position, at the cost of that wait
applying even when the device had already been off for a while. An instance
whose power cycle genuinely needs to differ from that composition supplies its
own `PowerCycleSequence`, the same way `RebootSequence` overrides the default
composition described under Systemd targets, below.
...
and concurrency group here is specific to this drive, not a value resolved
against a shared template, because no template exists. `Reboot` and `HardReboot`
are not shown here because a drive does not need them: both default to the
`PowerOnSequence`/`PowerOffSequence` composition described under Systemd
targets. A device whose reboot needs to differ from a plain power cycle would
add a `RebootSequence` and/or `HardRebootSequence` field with its own ordered
operation list, the same way `PowerOnSequence` and `PowerOffSequence` are given
here.

A corresponding entity-manager schema change is needed to accept
`Inventory.Item.Drive` and `Configuration.PowerSequence` (including the closed
operation vocabulary as a validated shape) as Board interfaces.
...
actually present. Each instance's operation sequences travel with that
instance's own entity-manager configuration, not as a separately shipped
artifact.
...
unless the instance's own entity-manager configuration supplies a
`RebootSequence` or `HardRebootSequence`, the same way phosphor-state-manager's
existing host daemon maps `GracefulWarmReboot`/`ForceWarmReboot` onto the plain
reboot target when a platform does not support a distinct warm reboot: a
per-instance default the daemon resolves, not a systemd unit dependency. A
device whose reboot genuinely differs from a power cycle, for example a terminus
command instead of toggling power, supplies its own `RebootSequence` or
`HardRebootSequence` field for that instance, overriding the default. No target
declares `Wants=` on another target for this: each target is started only after
the daemon has already run whatever sequence that transition resolved to, so a
dependency between targets would not do anything the daemon's own resolution has
not already done.
...
parameters, and phosphor-state-manager would hold the named template's step
list, shared across every instance that names it. This keeps the operation
vocabulary exactly as closed, and does not by itself put platform-authored
scripts or systemd wiring anywhere, so it would not violate the same
control-flow ownership the chosen design preserves. The reasons against it are
practical rather than architectural: it requires phosphor-state-manager's own
repository to accumulate every platform's sequence content, and it re-opens a
question entity-manager has already answered for itself. Entity-manager's own
README accepts duplicated configuration across per-model files as a deliberate
trade-off, valuing the ability to see every supported device's full
configuration in one place over avoiding repetition, and states that entity
manager does not participate in managing any device, only publishes data for a
reactor to consume. Writing each instance's sequence directly into its own
entity-manager configuration, rather than introducing a name that resolves
against a template kept elsewhere, follows that precedent instead of re-deciding
it. This design does not rule out a platform choosing to generate several
similar instances' sequence arrays from one source at its own
configuration-authoring time; that stays entirely outside
phosphor-state-manager's or entity-manager's concern.
