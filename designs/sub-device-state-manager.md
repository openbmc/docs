...
3. Separate hardware-specific power control (GPIO toggling, I2C IO expander
   ...
   design uses systemd targets as the extension point: platforms attach
   hardware control services via `.wants/` directories.
...
6. Per-type requirements such as host dependency are expressed through per-type
   ...
   `Inventory.Decorator.ManagedHost` interface), not imposed on all device
...
The entity-manager schema must be extended to accept `Inventory.Item.Drive`
...
to. This is the same interface already used by multi-host platforms (e.g.
Yosemite4 retimer configs) and consumed by bmcweb for system collection
enumeration. Board entities without `ManagedHost` receive no host-dependency
check and behave as host-independent devices.
...
- Single daemon: one entity-manager subscription, one event loop, one
  ...
  meson options are used for per-platform selection.
