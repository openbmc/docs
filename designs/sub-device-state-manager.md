...
5. All sub-device types share a single `State.Device` interface for transition
   ...
   watch, whether a host-dependency check applies, and the transitions to
   advertise; the transition-notification targets (see Systemd targets) are
   generic across every type and need no per-type naming. This is a guarantee
   about the power-control mechanism only; a device-specific interface for
   properties outside it is a separate, additive possibility (see Background)
   and is not ruled out here. Put differently, this is about adding a _type_; an
   instance whose sequence only recombines existing operations needs only its
   own entity-manager configuration, but a genuinely new kind of operation is a
...
within the single process, so there is no per-device daemon or service instance:
every device reuses the same generic systemd target templates. The first
supported type is drives. Adding a new device type (NIC, PCIe card) to power
control adds a table entry and the corresponding entity-manager configuration;
it requires no new interface, no new binary, and no new systemd target units,
since the transition-notification targets are already generic across every type.
...
any other platform hook for this path; no specific need for one has been raised
for sub-devices, and this design does not add an extension point ahead of such a
need.
...
sub-device types (NIC, PCIe) add a table entry to the same daemon and reuse the
existing generic target units.
