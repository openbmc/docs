...
- Single type-agnostic daemon (chosen): with one `State.Device` interface for
  ...
  type-specific behaviour reduces to three pieces of configuration per type (the
  inventory interface to watch, the target prefix, the host-dependency flag),
  with the individual devices discovered from entity-manager at runtime. A
  process boundary per type would therefore not be separating distinct logic.
  One subscription, event loop and association lifecycle regardless of how many
