...
- Compiled-in: platform-specific operations (GPIO, I2C, NVMe-MI) are compiled
  ...
  implementations.
- Systemd targets (chosen): platform hardware code stays out of the upstream
  source tree entirely. Platforms attach services to framework-defined targets
  in their meta-layer without modifying phosphor-state-manager, matching the
  existing host and chassis power control pattern, at the cost of losing
  compile-time type safety across the platform boundary.
