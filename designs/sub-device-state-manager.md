...
design adds `On` and `Off` transitions through a shared
`State.Decorator.PowerTransition` interface, adds a `paths:` metadata block to
`State.Drive` for the object path convention, and retains `State.Drive` for
