...
Transition targets (started by the daemon after the matching operation sequence
completes, see Sequence Execution):
...
Composite transitions ultimately trigger the poweron sequence through the same
target dependency ordering shown above.
