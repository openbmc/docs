...
whatever grouping it is given. A group acquired within a retry's sub-sequence or
a transition's sequence is always released by the daemon when that sub-sequence
or sequence ends, whether it succeeded, failed, or a retry's count was
exhausted; a platform's configuration only chooses which operations reference a
group, never whether the daemon gives it back.
...
statement that some sequence exists for that transition: an instance's
entity-manager configuration must supply the sequence for every transition its
type advertises (a `PowerOnSequence`/`PowerOffSequence` pair for `On`/`Off`, and
a `RebootSequence`/`HardRebootSequence` for `Reboot`/`HardReboot` if the type
advertises them). The daemon rejects creating a state object for an instance
missing a required sequence rather than advertising a transition it cannot
actually run.
...
may declare a named hook slot (`on_success`, `on_failure`) that the daemon
starts as part of the same job instead of, not in addition to, the transition
target itself, which is where a platform-specific companion action, such as a
recovery service after a failure, still attaches without becoming part of the
default path.
