...
design uses a shared `State.Decorator.PowerTransition` interface for all
transitions including new `On` and `Off` values, adds a `paths:` metadata block
to `State.Drive` for the object path convention, and retains `State.Drive` for
...
defines its own transition property (e.g. `RequestedDriveTransition` in
...
Redfish uses a single `ResetType` enum (defined in the Resource schema)
consistently across all device types (Drive, Port, Processor, PowerSupply,
Switch, and others), so a shared transition interface better matches the Redfish
model.
