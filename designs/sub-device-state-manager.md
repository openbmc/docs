...
and `CurrentDriveState` with the same pattern used by Host and Chassis. This
design introduces a shared `State.Decorator.PowerTransition` interface for
transition requests across sub-device types, while `State.Drive` retains
type-specific properties such as `CurrentDriveState`.
...
5. Core infrastructure (entity-manager discovery, association lifecycle, systemd
   ...
   All sub-device types share a single `State.Decorator.PowerTransition`
   interface for transition requests, consistent with Redfish using one
   `ResetType` enum across all device schemas. Adding a new type (NIC, PCIe
   card) requires producing a new binary on top of the shared layer, defining a
   per-type interface for type-specific properties, and installing a new systemd
   service; it does not require a new transition interface or reworking existing
   types.
6. Per-type requirements such as host dependency are expressed through per-type
   mechanisms (daemon-level host state monitoring using the existing
   `Inventory.Decorator.ManagedHost` interface), not imposed on all device types
   by the framework.
...
Sub-device types share a single `State.Decorator.PowerTransition` interface for
power control requests. This is consistent with Redfish, where all device
schemas reference the same `ResetType` enum from the Resource schema rather than
defining per-type reset actions. Existing `State.Host` and `State.Chassis`
retain their established transition interfaces; the same pattern could be
adopted for those types in a future revision.
...
- the shared `State.Decorator.PowerTransition` interface with per-type
  `AllowedTransitions` values
- the per-type PDI interface for type-specific properties (e.g. `State.Drive`
  for `CurrentDriveState`, `Rebuilding`, `LastRebootTime`)
...
card) means defining a per-type interface for type-specific properties,
producing a new binary that links the shared layer and declares its
`AllowedTransitions` subset, and installing a new service file. No new
transition interface is required.
...
/xyz/openbmc_project/state/drive/<instance>
  xyz.openbmc_project.State.Decorator.PowerTransition
    RequestedTransition       (enum: On, Off, Reboot, HardReboot, Powercycle)
    AllowedTransitions        (set[enum])
  xyz.openbmc_project.State.Drive
    CurrentDriveState         (enum: Ready, Offline)
    Rebuilding                (bool)
    LastRebootTime            (uint64, epoch time)
...
all drives associated with that host, updating `CurrentDriveState` to Offline.
This is built-in behavior that requires no platform-specific systemd wiring.
When a `RequestedTransition` of `On`, `Reboot`, or `Powercycle` is received
while the associated host is off, the transition is rejected with a D-Bus error,
which bmcweb surfaces as a Redfish 4xx response.
...
Transition targets (started by the daemon on `RequestedTransition`):
...
sets `RequestedTransition` on the `State.Decorator.PowerTransition` interface.
The Redfish `ResetType` to D-Bus transition mapping:
...
`AllowedTransitions` property on the state object.
...
- Single daemon: one entity-manager subscription, one event loop, one
  ...
  All types share one process, and the binary links every supported type unless
  meson options are used for per-platform selection. A single binary also
  reduces overall compressed image size (duplicated code defeats `-Osize`),
  lowers per-process overhead (host power state tracking, service metadata,
  startup logs), and improves QEMU emulation efficiency.
- Per-type binaries (chosen): each type has an independent lifecycle and
  recipe-level selection, matching the existing phosphor-state-manager and
  dbus-sensors conventions. The entity-manager subscription and event loop are
  replicated per binary, which is marginal since each binary filters on a
  different inventory item interface.
...
- Compiled-in: platform-specific operations (GPIO, I2C, NVMe-MI) are compiled
  ...
  and allows maintainers to refactor freely, at the cost of coupling platform
  hardware code into the state manager source tree. A similar pattern is used in
...
- Systemd targets (chosen): platform hardware code stays out of the
  ...
  phosphor-state-manager, matching the existing host and chassis power control
  pattern, at the cost of losing compile-time type safety across the boundary.
...
**Per-type transition interface for each sub-device type.** An earlier revision
of this design defined `State.Drive` with its own `RequestedDriveTransition`
property, following the existing per-type PDI convention (State.BMC, State.Host,
State.Chassis each with independent transition property names). Redfish uses a
single `ResetType` enum (defined in the Resource schema) consistently across all
device types (Drive, Port, Processor, PowerSupply, Switch, and others), so a
shared transition interface better matches the Redfish model.
...
while the drive state remains unchanged. The chosen design handles host
dependency entirely at the daemon level, as described in the Device Type: Drive
section.

## Impacts

phosphor-dbus-interfaces gains a new `State.Decorator.PowerTransition` interface
(`RequestedTransition`, `AllowedTransitions`). `State.Drive` retains only
type-specific properties (`CurrentDriveState`, `Rebuilding`, `LastRebootTime`).
...
The reset handler writes `RequestedTransition` on
`State.Decorator.PowerTransition`, so future sub-device types reuse the same
handler without new bmcweb code. No existing Redfish routes are affected. The
Drive.Reset action is absent on systems that do not deploy the daemon.
...
- Which repositories are expected to be modified?
  - `phosphor-dbus-interfaces` (new `State.Decorator.PowerTransition` interface;
    `State.Drive` type-specific properties)
