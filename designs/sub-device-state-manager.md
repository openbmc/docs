...
requests and current state. This is consistent with Redfish, where all device
...
type-agnostic: every type binds to the same `State.Device` interface, with one
property set and one pair of enums, so nothing in the daemon's D-Bus layer
varies by type.
...
What remains type-specific is a table of configuration data, not code. Each
entry supplies:
...
- Per-type binaries: with a separate `State.*` interface per type, each type
  would own its own generated D-Bus binding, with its own property names,
  transition enum and state enum. That binding is the boundary a separate
  process exists for. The entity-manager subscription, event loop, association
  lifecycle and `CurrentHostState` subscription are also replicated per process.
- Single type-agnostic daemon (chosen): with one `State.Device` interface for
  every type (see Background), that binding no longer exists. Every type binds
  to the same generated interface, so what stays type-specific is three pieces
  of data per type (the inventory interface to watch, the target prefix, the
  host-dependency flag) rather than code, and a process boundary per type would
  not be separating distinct logic. One subscription, event loop and association
...
Verification is performed against the phosphor-state-manager implementation on a
single-host platform, covering:
...
- each transition starting the matching target, and `CurrentDeviceState`
  following job completion through `TransitioningToOn` or `TransitioningToOff`
  to `On` or `Off`
- marker targets resolving initial state across a daemon restart, including the
  case where neither marker is active and the state resolves to `Unknown`
