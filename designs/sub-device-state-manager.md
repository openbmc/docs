...
Drives associated with a host through `ManagedHost` depend on that host's power
state: when the host is off, the drive state must reflect off, and a request to
power on the drive must return an error. Drives without `ManagedHost` are
host-independent, as described below.
