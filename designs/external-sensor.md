...
*   All of the existing sensor daemons obtain the sensor values they publish to
    D-Bus by reading from local hardware (typically by reading from virtual
    files provided by the _hwmon_[^3] subsystem of the Linux kernel). None of
    the daemons are currently designed with the intention of accepting values
    from an external source. Although there is debugging functionality for this,
    it is not the primary purpose for which they were designed.
...
The timeout feature gives something for the main loop to do, and is the primary
feature which distinguishes _ExternalSensor_ from other sensor daemons. Once an
external source starts providing updates, the external source is expected to
continue to provide timely updates. Each update will be properly published onto
D-Bus, in the usual way done by all sensor daemons, as a floating-point value.
...
*   "Units": String. This is a new parameter, unique to _ExternalSensor_. As
...
*   "Timeout": Numeric. This is a new parameter, unique to _ExternalSensor_. It
    is the timeout value, in seconds. If this amount of time elapses with no new
    ...
    NaN, as described above. This field is optional. If not given, the timeout
...
*   "Thresholds": Another JSON dictionary. This field is optional. It is passed
    along to the main _Sensor_ class during initialization, similar to other
    sensor daemons, and is not otherwise used by _ExternalSensor_.

*   "PowerState": String. This field is optional. It is treated similarly to
    "Thresholds".
...
is that the existing _Sensor_ base class does not currently allow its existing
D-Bus setter hook to be customized. This feature is straightforward to add[^23].
Among other things, the existing _Sensor_ class currently drops updates that are
duplicates of the existing sensor value. For use with _ExternalSensor_, we want
to recognize all updates received, even duplicates, as they are important to pet
the watchdog, to avoid inadvertently triggering the timeout feature. However, it
is still important to avoid needlessly sending the D-Bus _PropertiesChanged_
event for duplicate readings.

The timeout value was originally a compiled-in constant. If _ExternalSensor_ is
to succeed as a general-purpose tool, this must be configurable. It is
straightforward to add another configurable parameter[^24] to accept this
timeout value, as shown in "Parameters" above.

The hardest task of all, however, is getting it accepted upstream. If you are
