...
The eMMC in the Aspeed board has the following sections:
- Boot Area Partition (x2)
- RPMB Partition
- General Purpose Partition (x4)
- User Data Area

In general, it is assumed the eMMC has a boot hardware partition (Boot Area)
that is exposed as a different block device than the User Data Area.
...
- Filesystem layout: The root filesystem would be hosted in a read-only volume.
  ...
  through code updates. The /home directory needs to be writable to store user
  data such as ssh keys, so it would be a bind mount to the read-write volume.
  A bind mount is more reliable than an overlay, and has been around longer.
  Since there are no contents delivered by the image in the /home directory, a
  bind mount can be used. On the other hand, the /etc directory has content
  delivered by the image, so it would be an overlayfs to have the ability
...
- FAT MBR partitioning: FAT is a simple and well understood partition table
  ...
  depends on the prior partition. However, we have a demand for at most 4
  partitions.
