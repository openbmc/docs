...
## Proposed Design
- For secure boot of the root filesystem, use [dm-verity][], which provides the
  kernel with a way to verify the content of the root filesystem as it is read
  from storage.

  dm-verity only exposes verified blocks with no provision to change them.
  Therefore the rootfs will be distributed as a read-only file system (see also
  "Filesystem layout" below).
...
- The dm-verity parameters will be embedded in the FIT.
  ...
  The dm-verity parameters will be stored in the FIT, via the [bootconfig][]
  kernel feature that is appended to the initramfs. This allows to set the
  parameters during build time without having to patch the kernel.

  The dm-verity mapper exposes a range of blocks on a block device as a device
  mapper block device. To provide secured storage, a second range of blocks is
  used to store a hash tree, which may be on the same or a different block
  (partition). These block numbers, along with the hash algorithm, a salt, and
  the initial root hash are all required to access the root file system. The
  parameters must be passed securely to the kernel; the images in the FIT
  (including the kernel) are signed and verified by U-Boot.
...
- All Code update artifacts combined into a single image.
  ...
  has not been merged, last updated in 2018, two years ago).
...
- Store the dm-verity parameters either as a fdt overlay to be exposed via the
  device tree or through a ramdisk file to be combined into the initramfs
  (Ramdisk overlay is expected by the kernel but not in the current U-Boot FIT
  documentation, while device tree overlays are documented and supported).
  Alternatively, two short U-Boot scripts can set the bootargs parameter with
  the dm-verity invocation, one script for each of the two image partitions on
  the eMMC.
