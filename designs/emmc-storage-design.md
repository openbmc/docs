...
- Read-write ilesystem: ext4. This is a stable and widely used filesystem for
  eMMC. See the Alternatives section below for additional options.

- Static partitioning for the eMMC: This is chosen over dynamic partitioning
  due to the lack of offline tools to build an image (see Logical Volumes in
  the Alternatives section below). Two volumes to hold two roofs versions
  would be created with well known names such as A and B which would be
  populated during a code update, instead of creating volumes on the fly.
  This would allow U-Boot to know the name of the available rootfs volumes to
  boot from.
...
- Code update: Support two versions on flash.
