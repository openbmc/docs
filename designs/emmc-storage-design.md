...
- Partitioning: Two volumes to hold two roofs versions would be created with
  well known names such as A and B which would be populated during a code
  update, instead of creating volumes on the fly. This would allow U-Boot to
  know the name of the available rootfs volumes to boot from.
...
- Provisioning: The eMMC vendor would be provided with an OpenBMC image that can
  ...
  task. Therefore, have the initramfs create the LVM volume and use fetch the
  rootfs file into tmpfs from an external source to flash the volume. The
  rootfs file can be fetched using DHCP, UART, USB key, etc. A different option
  would be to build the image from QEMU, this would require booting QEMU as part
  of the build process to setup the LVM volume and create the image file.
