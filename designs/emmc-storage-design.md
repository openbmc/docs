...
- For secure boot, use [dm-verity][], which provides the kernel with a way to
  verify the content of the root filesystem as it is read from storage.
...
- Static partitioning for the eMMC: This is chosen over dynamic partitioning
  ...
  The eMMC would be divided using the device-mapper linear target, which allows
  for the expanding of devices if necessary without having to physically
...
- The dm-verity parameters will be embedded in the FIT.
  ...
  The dm-verity parameters will be stored in the FIT, either as a fdt overlay
  to be exposed via the device tree or through a ramdisk file to be combined
  into the initramfs (Ramdisk overlay is expected by the kernel but not in the
  current U-Boot FIT documentation, while device tree overlays are documented
  and supported). Alternatively, two short U-Boot scripts can set the bootargs
  parameter with the dm-verity invocation, one script for each of the two image
...
- All Code update artifacts combined into a single image.
  ...
  has not been merged, last updated two years ago in 2018).
...
- FAT MBR partitioning
  ...
  The read-write filesystem occupies in partition 4.
