...
- At the start of PLDM daemon it invokes the discovery of PLDM endpoints by
...
in case the PLDM endpoint does not support setTID command. The bmc should
...
## PLDM endpoint lifecycle management
 A separte daemon/handler is created to add newly discoverd
endpoints in the PLDM device dbus object when they are available.
 It also monitors for PLDM endpoint is non availability of endpoints
 at runtime  and it should depopulate the PLDM endpoint from the DBUS object.

 Two separate endpoint lifecycle handlers should be created for MCTP based
 PLDM endpoints as well as NCSI over RBT based PLDM endpoints.

 In case the PLDM endpoint config is mentioned as static config then
 the PLDM endpoint shall not be supported.

### MCTP PLDM endpoint lifecycle manager
- The MCTP daemon supports discovery and population
   of endpoint details on  dbus.
-  The MCTP based PLDM endpoint lifecycle handler uses the MCTP endpoint discovery
  api to dynamically handle addition and removal of endpoints.

### NCSI PLDM endpoint lifecycle manager
- when PLDM daemon triggers the discover endpoints at startup
this NCSI PLDM endpoint lifecycle manager handler is started.
- It is responsible for adding the discovered PLDM endpoints over PLDM endpoint
- If the NCSI endpoint supports async event notification for PLDM endpoint
   removal, then a handler is implemented to depopulate the respective
   PLDM endpoint from the PLDM endpoint dbus object.
- In case the PLDM endpoint discovery is static configuration then
 The PLDM endpoint lifecycle management is not handled
