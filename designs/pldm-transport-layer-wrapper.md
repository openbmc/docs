# PLDM transport layer abstraction
...
- The supported PLDM types are as follows :
     PLDM type 0, PLDM type 2 and PLDM type 5.

## Transport layer identification

### Terminus id discovery and EID mapping

In order to make the transport layer abstracted from the PLDM  app,
...
service is started.
The discovery part of the wrapper is responsible for identifying PLDM
endpoints based on NCSI over RBT transport layer.

The terminus management part is as per the pldm-stack design
(https://gerrit.openbmc.org/c/openbmc/docs/+/47252/18/designs/pldm-stack.md).

Anyway the terminus management process should handle management of MCTP as well as
NCSI over RBT PLDM endpoints.

When  calling the PLDM requester api, the terminus id(pldm_tid_t) is passed
...
- The NCSI parameters are read as static config which is stored in json file read
  by PLDM daemon at startup.
  A virtual terminus id is set for each individual NCSI PLDM device.

Example NCSI over RBT PLDM sensor static config :
...
### NCSI link availability
- PLDM daemon can  use the NCSI capability interface to check
the current NCSI package and channel before discovering PLDM endpoints
via NCSI over RBT transport layer.

ref design : https://gerrit.openbmc.org/c/openbmc/docs/+/55959
...
The PLDM requester class has registerRequest() method
...
+-------------------+     +----------------+      +---------------+    +----------------+
|                   |     |                |      | PLDM transport|    |                |
| PLDM daemon       |     |     PLDM       |      |  wrapper /    |    |   transport    |
|                   |     |                |      |   Transport   |    |   layer api    |
|                   |     |  requester     |      |   abstraction |    |                |
|                   |     |    class       |      |   layer       |    |( NCSI or MCTP) |
+--------+----------+     +-------+--------+      +-------+-------+    +--------+-------+
         |                        |                       |                     |
         |                        |   discoverTids()      |                     |
         +------------------------+----------------------->                     |
...
- The PLDM client apps which calls the PLDM requester with terminus id
...
- When the PLDM message is received the callback function
  ...
  Note : Instance id is used to identify the respective response for the
  request.

## Impacts

The current PLDM apps using MCTP based socket and EID will need to use the
