...
```bash
curl -c cjar -k -X POST -H "Content-Type: application/json" -d '{"data": [LockType:"<write/read>",Segmentflags["lockFlags":<Lockall/Dontlock/LockSameSegement>,"Segmentlength":"1"....],ResourceID:"String",..] }' http:/{BMC_IP}/ibm/v1/host/locks/Acquirelock
```
...
```bash
curl -c cjar -k -X POST -H "Content-Type: application/json" -d '{"data": [[RequestID,..]"] }' http:/{BMC_IP}/ibm/v1/host/locks/ReleaseLock
```
...
```bash
curl -c cjar -k -X POST -H "Content-Type: application/json" -d '{"data": [SessionID],..}' http:/{BMC_IP}/ibm/v1/host/locks/GetLockList
```
