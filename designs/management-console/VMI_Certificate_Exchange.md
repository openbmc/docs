...
## Problem Description
HMC needs to establish a secure connection to VMI for virtualization
management.
...
IBM systems can run both IBM specific host-firmware(PHYP) and the KVM.
...
HMC can query BMC state and run this rest command to initiate certificate
...
- Create new interface SignCSR in webserver which takes CSR string as input and returns
  certificate string.
- PLDM commands needs to be defined
  This method invokes PLDM commands to forward CSR request to VMI and get signed
  certificates from VMI.PLDM command needs to be defined.
- BMC is just passtrough which allows certificate exchange between VMI and HMC.
...
- Build time configure variable defined to control enable and disable of this API
  in webserver.Its required only for IBM systems with IBM specific host-firmware(PHYP)
- It does not impact security of BMC
...
- Certificate exchange fails in the following scenarios
  ...
  * If PHYP throws error for certificate validation.
  This interface returns HTTP error code Internal server error(500).
