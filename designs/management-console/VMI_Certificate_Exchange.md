...
curl -k -H "X-Auth-Token:  <token>" -X POST "Content-Type: application/json" -d
  '{"CsrString":"<CSR string>"}' http://{BMC_IP}/ibm/v1/VMInterface/SignCSR
```
...
```
```bash
curl -k -H "X-Auth-Token:  <token>" -X GET "Content-Type: application/json"
  http://{BMC_IP}/ibm/v1/VMInterface/GetRootCertificate
```
...
- Create new interface SignCSR in webserver which takes CSR string as input and returns
  certificate string.This method invokes PLDM commands to forward CSR request to VMI and
  get signed certificates from VMI.
- Decided to use PLDM file IO approach for certificate exachange
