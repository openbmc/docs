...
curl -k -H "X-Auth-Token:  <token>" -X POST "Content-Type: application/json" -d
  '{"CsrString":"<CSR string>"}' http://{BMC_IP}/ibm/v1/Host/SignCSR
```
...
```
```bash
curl -k -H "X-Auth-Token:  <token>" -X GET http://{BMC_IP}/ibm/v1/Host/GetRootCertificate
```
...
- Files 'CSR_<random number>','ClientCert_<random number>','RootCert_<random number>' will
  be created  for PLDM to read and write client and root certificates under '/var/lib/bmcweb'.
  integer random number appended to file name to serve multiple request at the same time.
- Implement Dbus methods to share file descriptors of these files with PLDM.
  PLDM does read or write on these file descriptors.
- Create new interface SignCSR in webserver which takes CSR string as input and returns
  certificate string. This method invokes PLMD file IO 'NewFileAvailable' command of
  CERT_SIGNING_REQUEST type to forward CSR request to VMI and does inotify on
  '/var/lib/bmcweb/ClientCert_<random number>' file to get signed certificate.
- Create new interface GetRootCertificate in webserver which reads root certificate from
  '/var/lib/bmcweb/RootCert_<random number>' file.
