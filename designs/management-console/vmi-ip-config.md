nterface IP Address Configuration
...
#### Get the Host network details

Request:
```
curl -k -H "X-Auth-Token: $bmc_token" -X GET
https://${bmc}/ibm/v1/VMInterface/network
```
Response:
...
```
{
  "data": {
    ...
    "/ibm/v1/VMInterface/network/<IntfId>": {
      "DHCPEnabled": true/false,
      "PhysicalLocationCode": ""
    },
    "/ibm/v1/VMInterface/network/<IntfId>/ipv4/<objectId>": {
      "Address": "",
      "Origin": static/dynamic,
      "PrefixLength": <>,
      "Type": "IPv4"
    },
  "/ibm/v1/VMInterface/network/<IntfId>/ipv6/<objectId>": {
      "Address": "",
      "Origin": static/dynamic,
      "PrefixLength": <>
      "Type": "IPv6"
...
-H "X-Auth-Token: $bmc_token -X POST https://${bmc}/ibm/v1/VMInterface/network/<IntfId>
-d '{"data": ["IPv4", "address", "prefixlength"]}'
```
Response:
...
```
{
  "data": "ibm/v1/VMInterface/network/<IntfId>/ipv4/<objectID>",
  "message": "200 OK",
  "status": "ok"
}
```
...
-d "{"DHCPEnabled" : true}" https://${bmc}/ibm/v1/VMInterface/network/<IntfId>
```
...
https://${bmc}/ibm/v1/VMInterface/network/<IntfId>/ipv4/<objectId>
```
Response:
...
When the system is powered on initially, HMC should setup the VMI interface to PHYP via BMC , provided the DHCP is turned off. Since the Power Hypervisor only supports a single IP address, it is recommended to only configure one IP address.
...
Redfish Interface commands to GET and PATCH the host interface parameters.
