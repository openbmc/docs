...
in their D-Bus path is unique. Failure to do so would result in the inventory
not being published in Redfish, as inventory-consuming applications such as
openbmc/bmcweb are specifically designed to identify resources solely based on
leaf nodes.

For instance, if we wish to expose both adapters in Redfish, an inventory
hosting application cannot have two distinct D-Bus objects with the following
path hierarchy:
