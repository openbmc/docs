...
If it fails to send the events(logs or metric reports), it will retry for
number of attempts configured in Event Service "DeliveryRetryAttempts" with
an interval of "DeliveryRetryIntervalSeconds". During retry attempts, all
the new events(logs or metric reports) will be queued in async timer and
send will be attempted if connection comes alive. If the failure persist,
even after the retry, it will delete the subscription information. User can
re-subscribe for events if this happens. This is requirement as per current
