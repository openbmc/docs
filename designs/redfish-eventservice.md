...
High level requirements:
  ...
  - BMC can provide functionality to send out test events.
  ...
  - SSE: BMC shall respond to GET method on URI specific in "ServerSentEventUri"
    ...
    status code 400, if filter parameters are not supported or 404 if filter
    parameter contains invalid data. Also SSE subscription data can not be
  ...
  - BMC should accept the "Last-Event-ID" header from the client to allow a
  ...
  - To avoid exhausting the connection resource, BMC shall implement a
    restriction of maximum 20 connections (Maximum of 10 SSE connections).
  - BMC should log an redfish event message for subscription addition, deletion
    and modification.

## Proposed design

Redfish event service provides way to subscribe for specific kind of events or
MetricReport and asynchronously send subscribed type of events or reports
to client when ever it occurs.

Two styles of events are supported OpenBMC EventService.

  1. Push style events (http/https): When the service detects the need to send
...
Push style events(HTTP/HTTPS) flow diagram:
...
+-----+-------+   +-----+------+               +--+--------+--------+---------+------+ +-----+-----------+
      |                 |                         |        |        |         |              |
_______________________________SUBSCRIBE FOR EVENT________________________________________________________
      |                 |                         |        |        |         |              |
      |                 |                         |        |        |         |              |
      |                 |    EVENT SUBSCRIPTION   |        |        |         |              |
      +------------------------------------------>+        |        |         |              |
      ...
      |                 |                         |        |        |         |              |
_______________________________SEND EVENTS/METRICS WHEN GENERATED______________________________________
      |                 |                         |        |        |         |              |
      |                 |                         |        |        |         |              +----+
      |                 |                         |        |        |         |              |LOOP|
      |                 |                         |        |        |         |              +<---+
      ...
      |       |         |                         |        |FORMATTED DATA    |              | |
      |       |         |                         |        |        |         |              | |
      |       |         ___________ FIRST TIME, CREATE CONNECTION ____________               | |
      |       |         |                         |        |        |         |              | |
      |       |         <---------------------------------->        |         |              | |
      ...
      |       | |-+----+|                         |        |     |  |         |              | |
      |       | | ^_____________ SEND FAILED ____________________|  |         |              | |
      |       | | |     |                         |        |     |  |         |              | |
      |       | | |     <---------------------------------->     |  |         |              | |
      |       | | |     |  ESTABLISH HTTP/HTTPS   |        +----+|  |         |              | |
      |       | | |     |     CONNECTION          |        |REOPEN  |         |              | |
      ...
      |       | | |     |                         |        |<---+|  |         |              | |
      |       | | +------------REPEAT SEND LOOP------------------|  |         |              | |
      |       | | ______________ SEND SUCCESS_____________ |     |  |         |              | |
      |       | |       |                         |        |---+ |  |         |              | |
      |       | |       |                         |        |POP| |  |         |              | |
      |       | |       |                         |        |QUEUE|  |         |              | |
      |       | |       |                         |        |<--+ |  |         |              | |
      ...
      |       | |       |                         |        |<---+|  |         |              | |
      |       | |       |                         |        |     |  |         |              | |
      |       | | ______________ REACHED THRESHOLD LIMIT ________|  |         |              | |
      |       | |       |                         |        |----+|  |         |              | |
      |       | |       |                         |        |DELETE  |         |              | |
      |       | |       |                         |        |STORE|  |         |              | |
      |       | |       |                         |        |&QUEUE  |         |              | |
      ...
      |       +--------------------------------------------------------------------------------+
      |                 |                         |        |        |         |              |
________________________________DELETE EVENT SUBSCRIPTIONS __________________________________________
      |                 |                         |        |        |         |              |
      +-----------------+------------------------>+        |        |         |              |
      |        DELETE SUBSCRIPTION                |        |        |         |              |
      |                 |                         +------+ |        |         |              |
...
_______________________________SEND EVENTS/METRICS WHEN GENERATED______________________________________
      ...
      |       | |LOOP  ||   SEND FORMATTED DATA   |        |     |  |         |              | |
      |       | |+-----+|                         |        |     |  |         |              | |
      |       | ||______________ SEND FAILED ____________________|  |         |              | |
      |       | ||      |                         |        |     |  |         |              | |
      |       | |+-------------REPEAT SEND LOOP------------------|  |         |              | |
      |       | |       |                         |        |     |  |         |              | |
      |       | | ______________ SEND SUCCESS_____________ |     |  |         |              | |
      ...
      |       | |       |                         |        |<---+|  |         |              | |
      |       | |       |                         |        |     |  |         |              | |
      |       | | ______________ REACHED THRESHOLD LIMIT_________|  |         |              | |
      |       | |       |                         |        |----+|  |         |              | |
      |       | |       |                         |        |DELETE  |         |              | |
      |       | |       |                         |        |STORE|  |         |              | |
      |       | |       |                         |        |& QUEUE |         |              | |
      ...
      |       +--------------------------------------------------------------------------------+
      |                 |                         |        |        |         |              |
________________________________DELETE EVENT SUBSCRIPTIONS ____________________________________________
      |                 |                         |        |        |         |              |
      +-----------------+------------------------>+        |        |         |              |
      |        DELETE SUBSCRIPTION                |        |        |         |              |
      |                 |                         +------+ |        |         |              |
      ...
      |                 |                         |    LOG REDFISH EVENT LOG  |              |
      |                 |                         |        |        |         |              |
________________________________ CLIENT CLOSE CONNECTION  ____________________________________________
      |                 |                         |        |        |         |              |
      |                 |    CLIENT CLOSED        |        |        |         |              |
      |                 |------------------------>|        |        |         |              |
      |                 |                         +-------+|        |         |              |
...
subscriptions data when ever user creates or updates or delete subscription.
...
(open/close) is differs for both subscription types such as 'Push style
...
In case of push style eventing, http/https connection will be established
...
handle and subscription map will be stored in event service sender.

In case of SSE, connection handle will be read from resource and used by
sender for sending events.

All the formatted data receieved, will be pushed to queue maintained under
sender against the connection-subscription map. Up on successful sending
of event data, that formatted data entry will be pop-ed from queue and
next formatted data will be attempted for send and so on.

If send events(logs or metric reports) fails, then it will retry for
...
if send fails due to some reasons, http/https connection will be
re-established. During retry attempts, all the new events(logs or metric
reports) will be pushed to queued in sender and send will be attempted if
connection comes alive. If the failure persist, even after the retry,
it will delete the subscription information. Same will be updated to resource
manager and logged to redfish event logs. User can re-subscribe for
events if this happens. This is requirement as per current redfish
specification.

Any temporary network failure during event notification, and persist till
threshold limit, will permanently remove the subscription. This may be
...
Note:
  ...
  1. In case of SSE, Client is initiator of connection and so SSE stream
     ...
     event to SSE client(For notifying SSE client to re-etsablishing
     connection).
...
subscriptions collection. Once bmcweb recieves the SSE stream request,
the open connection will be kept alive till the client/server closes
the connection. The "Context" property in the EventDestination resource
shall be an opaque string generated by the service.
...
to webserver running on external system. Also in case of SSE, connection
...
library. For https connection it uses the root CA certificate which is loaded
in certificate store. If certificate verification fails while establishing the
connection, it retries for configured retry attempts(DeliveryRetryAttempts) and
delete the subscription if it reaches threshold limit. Users can upload new
certificates if needed. More details about certificate can be found at
...
seems to be in-efficent because
...
configuration and subscriptions data. Also considered using settingsd
