...
```
{
  ...
  "ServerSentEventUri": "/redfish/v1/EventService/Subscriptions/SSE",
  "ServiceEnabled": true,
  "Subscriptions": {
...
Note:
  ...
  1. SSE stream connection can't be re-established when bmcweb service
     ...
     and can re-subscribe for SSE events using "ServerSentEventUri" once
     bmcweb becomes functional after restart or BMC reboot scenarios.
...
It shall support both http and https connections which will be created
using the boost::beast library. For https connection it uses the root CA
certificate which is loaded in certificate store. Users can upload new
