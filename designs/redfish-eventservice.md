...
[link](https://github.com/openbmc/docs/blob/master/redfish-logging-in-bmcweb.md)
     ...
     should figure out the way for getting above mentioned two requirement and
     can hook it to EventService design specified here.
...
Note:
  ...
  1. SSE stream connection can't be re-established when bmcweb service
     or BMC goes for reboot. While closing the bmcweb service all http
     connections will be closed and client will receive the close connection
     event. Client can re-subscribe for SSE events using "ServerSentEventUri"
     once bmcweb becomes functional after restart or BMC reboot scenarios.
