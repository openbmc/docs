...
| |                    | |Report Update        |                          |   |
| |                    <-+                     | +----------------------+ |   |
| |  GET on Metric     |                       | |Note that sensor's    | |   |
| |  Report URI        |                       | |PropertiesChanged     | |   |
| +-------------------->                       | |signal is asynchronous| |   |
| |       OR           |                       | |to metric report timer| |   |
| |  Send report as    |                       | |This timer is the only| |   |
| |  SSE if it was     |                       | |thing that triggers   | |   |
| |  configured this   |                       | |metric report update  | |   |
| |  way               |                       | +----------------------+ |   |
| <--------------------+                       |                          |   |
| |                    |                       |                          |   |
| |                    |                       |                          |   |
...
| |                    <-----------------------+                          |   |
| |                    |                       | +----------------------+ |   |
| |                    +-+Redfish Metric       | |Note that sensor's    | |   |
| |                    | |Report Update        | |PropertiesChanged     | |   |
| |                    <-+                     | |signal triggers the   | |   |
| |                    |                       | |report update. It is  | |   |
| |                    |                       | |sufficient that the   | |   |
| |  GET on Metric     |                       | |signal from only one  | |   |
| |  Report URI        |                       | |sensor triggers report| |   |
| +-------------------->                       | |update.               | |   |
| |       OR           |                       | +----------------------+ |   |
| |  Send report as    |                       |                          |   |
| |  SSE if it was     |                       |                          |   |
...
| |                    |                       | |sensor's readings.      |   |
| |                    |  Send Properties-     | |No additional sensor    |   |
| |                    |  Changed signal       <-+readings are performed. |   |
| | HTTP response      <-----------------------+                          |   |
| | with json payload  |                       |                          |   |
| <--------------------+                       |                          |   |
| |                    |                       |                          |   |
...
```MetricParams``` array entry is a structure containing:
...
properties (apart of standard D-Bus interface):
