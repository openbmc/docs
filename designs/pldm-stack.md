...
```text
/yaml/xyz/openbmc_project/File/Object.interface.yaml
...
properties:
    ...
    - name: Source
      ...
      default: Unknown
      description: >
          Source name of the file. Can be `System` if originated from the system.
    - name: Size
      type: uint64
      default: Unknown
...
methods:
    - name: Read
      description: Method to read the current file content.
      parameters:
          - name: Offset
            type: uint32
            description: >
                The requested offset of the file to start reading at.
          - name: Length
            type: uint32
            description: >
                The requested length in bytes of the content to be read from the file.
          - name: Exclusivity
            type: bool
            description: >
                The exclusivity the returned file descriptor holds on the file.
      returns:
          - name: Fd
            type: unix_fd
            description: >
                The file descriptor of the socket the file content was written to.
      errors:
...
`Read()` method elaborations:

1. `Read()` method will call `DfOpen` using `PDR::FileIdentifier` to File Host
   to get the fd, then use this fd to call `DfRead` to File Host, and `DfClose`
   after reading the desired data. `Read()` will write all the data to a memfd
   and return the fd as unix_fd type, so callers will have to save the data in
   the returned fd to their memory, or to the filesystem depending on their
   needs. `xyz.openbmc_project.Common.File.Error.Open` or
   `xyz.openbmc_project.Common.File.Error.Read` will be returned to the caller
   of `Read()`, if File Client fails to open or read the file from File Host.

2. `Read()` will read the requested section size from the file at the requested
   offset by setting `Offset` and `Length` parameters to
   `RequestedSectionOffset` and `RequestedSectionLengthBytes` fields
   respectively in the `DfRead` (or `MultipartReceive`) request for the first
   part (`TransferOperation = XFER_FIRST_PART`).
   `xyz.openbmc_project.Common.File.Error.Seek` will be returned if `Offset` is
   out of range. If the requested `Length` is larger than the current file size,
   the actual received data size will equal the current file size.

3. For serial FIFO file type, `Offset` is ignored and `RequestedSectionOffset`
   is always 0, because this type does not support seeking.
   `RequestedSectionLengthBytes` is the smallest between the negotiated part
   size resulted from the `NegotiateTransferParameters` request and the
   requested `Length`, so `Read()` will repeatedly call `DfRead` until `Length`
   is fulfilled or the data size retrieved from a `DfRead` is smaller than
   `RequestedSectionLengthBytes`, which means those are the last parts.

4. If `Exclusivity` parameter is enabled in `Read()`, `DfOpenExclusive` bitfield
   in the `DfOpenAttribute` will be set to 1 if the file's PDR has `ExReadOpen`
   bitfield enabled in `FileCapabilities` field. As `Read()` only reads part of
   the file content, it will not support `ZeroLength` bitfield in
...
- If the data is large, compared to the whole memory region allocated to the
  memfd object, with unix socket, clients can have control of each data chunk's
  size read from the socket. With chunking for socket, it can mitigates the
  memory and CPU costs associated with kernel buffering and data copying.

D-Bus error transfer:

- `pldmd` actually starts to read and write file data only when the socket fd is
  returned to callers (there's no problem in parsing the arguments). Therefore,
  `Read()` can't deliver D-Bus errors in error scenarios to inform what happends
  to the underlying interaction with the actual file, and callers have to rely
  on the error code (if any) returned from interactions with the socket.

- As soon as a memfd is returned from a `Read()`, data is already retrieved from
  the other terminus and written to the fd. In case of errors during executing
  PLDM File Transfer commands, `pldmd` can instead return D-Bus error to
  callers.

Client dependency:

- When a socket is returned to callers and `pldmd` starts reading from the
  actual file and writing to the socket, it needs to poll/select the socket to
  check readiness before-hand. If callers fail to read from the client side of
  the socket pair for too long and the kernel buffer is already full, `pldmd`
  needs to handle when to close the file descriptor to the actual file and
  unilaterally close the socket's server side because it can't hold the file
  descriptor open for too long, especially for exclusive open session, not to
  mention the the involvement of `DfHeartbeat` command in between to keep the
  session alive. If it does not further address this, it has to rely on the
  clients to coordinate.

- When a memfd is returned, data content is ready and placed in the memory
  region associated with it. Whether the client choose to read from it or not
  does not matter to `pldmd` as `Read()` method's job is now done.

Conclusion:

- Socket suits better for message-based or stream-based communication between
  processes. In this context, socket will help if we want `pldmd` to write the
  file content to the fd in small chunks, no matter how clients process the data
  afterwards.
