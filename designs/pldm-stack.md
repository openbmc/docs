...
- [xyz.openbmc_project.State.Decorator.Availability](https://github.com/openbmc/phosphor-dbus-interfaces/blob/master/yaml/xyz/openbmc_project/State/Decorator/Availability.interface.yaml),
  ...
  property is `true` when the terminus responds to the PLDM request that reads
  the sensor, and `false` otherwise (for example when the read command times out
  or returns a completion code other than `PLDM_SUCCESS`).
...
`State.Decorator.OperationalStatus`. The `Available` property is `true` when the
terminus responds to the PLDM request reading the component sensor, and `false`
otherwise. The `Functional` property is `true` only when the component sensor's
`sensorOpState` is `enabled`, and `false` for any other value. No object is
created for the state sensor itself; the composite sensor is a packaging of the
wire protocol and its component sensors stand alone on D-Bus.
...
the `PresentState`/`PreviousState` properties are set to `Unknown`. Supporting a
new state set requires adding its interface, aligned with DSP0249 1.4.0, to
