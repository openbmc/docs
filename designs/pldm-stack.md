...
reading all the content. `Read()` will write all the file content to a memfd and
return the fd as unix_fd type, so callers will have to save the data in the
returned fd to their memory, or to the filesystem depending on their needs.
