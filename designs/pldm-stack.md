...
```
Or the effecter index with the TID if the effecter does not have
the effecterAuxiliaryNames PDR and the Terminus does not have the configured
name.
```
...
Effecters come in 2 types: numeric effecters and state effecters with the
former sharing a common list of base units with numeric sensors but requiring
the ability to be written to. There's a need to bring up a separate set of
