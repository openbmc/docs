...
interface and `State.Decorator.OperationalStatus`, whose `Functional` property
derives from that component sensor's own `sensorOpState`. No object is created
for the state sensor itself; the composite sensor is a packaging of the wire
protocol and its component sensors stand alone on D-Bus.
...
adding its interface, aligned with DSP0249 1.4.0, to phosphor-dbus-interfaces
first.
