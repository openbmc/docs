...
- The pldmd asks the host firmware's PLDM stack for the host's FRU record table,
  by sending it the PLDM GetFRURecordTable command. The pldmd receives a PLDM
  FRU record table from the host firmware (
...
- Configurations can be written for entity-manager to probe an interface like
  xyz.openbmc_project.PLDM.FRU, and create FRU inventory D-Bus objects.
  ...
  example, xyz.openbmc_project.Invnetory.Decorator.Asset property values, such
  Part Number and Serial Number, in the entity manager configuration file.
  Bmcweb can find these FRU inventory objects based on D-Bus interfaces, as it
  does today.
