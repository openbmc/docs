...
set D-Bus interfaces below. There is no Redfish schema for a PLDM state sensor,
so `pldmd` publishes the state on the inventory D-Bus object of the entity
rather than on a D-Bus object of the state sensor.
