...
enumerations:
    - name: PurposeType
      ...
      values:
          - name: Diagnostic
            description: >
                This file provides diagnostic data.
          - name: Security
            description: >
          ...
          - name: Telemetry
            description: >
                This file stores telemetry data.
          - name: Directory
            description: >
                This is a directory.
          - name: OEM
...
File object path: `/xyz/openbmc_project/file/$TerminusName/$FileName`.
...
directory-classified file, will be placed one level behind the directory object
path (e.g `/xyz/openbmc_project/file/$TerminusName/$DirName/$FileName`), and the
directory object might not have to hold any interface or property.
...
reading all the content. `DfHeartbeat` will be sent to File Host before reading
each section (one section is from requesting the first part to acknowledgement)
to ensure the file descriptor availability within a desired period for regular
file type, whose data is preserved by default and size might become large,
compared to the serial FIFO file type. `Read()` will write all the file content
to a memfd and return the fd as unix_fd type, so callers will have to save the
data in the returned fd to their memory, or to the filesystem depending on their
