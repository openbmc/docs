...
2. `Read()` will read the requested section size from the file at the requested
   ...
   the actual received data size will equal the current file size. For serial
   FIFO file type, `Offset` is ignored and `RequestedSectionOffset` is always 0,
   because this type does not support seeking or multiple sections. `Length`
   should be set as the requestor part size in the `NegotiateTransferParameters`
   request before opening the session to serial FIFIO file. In this case
   `RequestedSectionLengthBytes` is always the negotiated part size resulted
   from the `NegotiateTransferParameters` request, so it can be smaller than the
   quested `Length`.

3. If `Exclusivity` parameter is enabled in `Read()`, `DfOpenExclusive` bitfield
   in the `DfOpenAttribute` will be set to 1 if the file's PDR has `ExReadOpen`
   bitfield enabled in `FileCapabilities` field. As `Read()` reads part of the
   file content, this will not support `ZeroLength` bitfield in
   ...
   sessions are not allowed and `DfOpenExclusive` not supported so `Exclusivity`
   is ignored.
