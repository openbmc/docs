...
- Transfer the boot log and diagnostic data of the termini such as SoC, PCIe
  Devices, DIMM to BMC. Those info then can be logged to Redfish and Ipmitool
  sel log.
- Transfer crash dump logs before the device turn off, the logs can be use to
  identify the device issue.

#### User interface

The Redfish interface `/redfish/v1/Systems/Self/LogServices/Dump` will be used
to as user interface to interact the File Host.
...
- The action
  ...
  the File Host. The input parameters to this action are
  `{"DiagnosticDataType": "OEM","OEMDiagnosticDataType": "<OptionNames>"}`. The
  `<OptionNames>` can be filed names, group name of files, the device name or
  oem defined name... This option will be handled by `file collector service`
  which can't be overided by OEM service so it can be OEM defined.

  This is non-blocking action. BMC will response for one Redfish request with
  the `<task_id>` while `file collector service` keeps querying the files from
  the File Host. To simplize the implementation, BMC will only handle the
...
from the File Host and tar the collected files to
...
service will use the file name to identify which the PLDM file needs collect.
The format of file name is <Terminus name>\*<File name>. Where `Terminus name`
is from Terminus's `Entity Auxiliary Names PDR` in section 28.18 of DSP0248
...
provided more advantaged than the D-Bus interface. With that interface, the file
...
requests and other sensors/events requests work consequently.
