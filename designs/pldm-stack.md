...
To identify PLDM terminus from all EIDs found in system, pldmd sends GetPLDMType
command to check if the EID supports PLDM type 2 and then pldmd should send
SetTID command to each found PLDM terminus to assign a unique Terminus ID.
...
polling during firmware updating to maximum bandwidth of physcial media).
