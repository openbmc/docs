...
enumerations:
    ...
    - name: SourceType
      ...
      values:
          - name: System
            description: >
                This file is originated from the system.
          - name: BMC
            description: >
...
- `Size` maps with the value read from the File Size Sensor associated with this
  ...
  Sensor.
- `Read()` method processes the input parameters and throws D-Bus errors if
  ...
  returning, it triggers an async task to manilpulate the actual file. As
  callers listen on the returned client socket, the async task reads the actual
  file and writes to the server socket. So, `Read()` finishes after the
  socketpair is setup and does not wait for completion of any action made to the
  actual file.

### Data read flow

The asynchronous task triggered as a result of `Read()` call will have the
following behaviors:
...
2. For regular file type, it reads the requested section size from the file at
   ...
   current file size, the actual received data size will equal the current file
   size.
...
Client -> pldm ++: 1.Call Read()]
pldm -> ss**: Setup socketpair
activate ss
pldm -> pldm: <<Async>> Trigger file reading\ncoroutine task
pldm --> Client --: return fd to the client socket
...
note over pldm: File reading task finishes
pldm -> fh ++: DfClose
fh --> pldm --
...
Therefore, `Read()` can't deliver D-Bus errors in error scenarios to inform what
