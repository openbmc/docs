...
`State.Decorator.Availability`, and `State.Decorator.OperationalStatus`. A
state sensor is initialized by the `GetStateSensorReadings` command after
discovery, and then updated by the `stateSensorState` (one component sensor)
and `sensorOpState` (all component sensors of the sensor) events, or by the
polling method below when the terminus cannot generate events. The reading
updates the state value exposed by `Sensor.Value`, and its `sensorOpState`
field updates the `Functional` property of `State.Decorator.OperationalStatus`
(`true` only when `sensorOpState` is `enabled`). If `pldmd` fails to get the
...
and the state value is set to `Unknown`. No object is created for the state
sensor itself; the composite sensor is a packaging of the wire protocol and its
