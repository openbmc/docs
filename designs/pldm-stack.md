...
Entity name will be taken as hash of ActiveComponentImageSetVersionString which
is part of inventory information.
Example: /xyz/openbmc_project/software/NIC0

Interfaces:
1. `xyz.openbmc_project.Software.Updateable`, which indicates whether the Update
Service can update the firmware or not. There could be some devices which
supports inventory command, but does not support PLDM firmware update.
Updateable property can be used to determine whether the device supports
