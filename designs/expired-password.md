...
3. There is a way for an account owner to change their own expired
   ...
   non-network vector to change the password.  For example:
    ...
    - SSH server: Implement the expired password change dialog.
...
only change the passsword and terminate the session.  The usage
pattern is:
 1. Create a session.
 2. If the PasswordChangeRequired message is present:
     a. PATCH the new password into the ManagerAccount object.
     b. Any other use of the session will get an authority failure.
     c. DELETE the Session object to terminate the session.
     d. Create a new session and continue.
...
has an expired password:
 - If the `/login` URI was used, this will indicate the password must
   be changed, and will not establish a session.  In this case, the
   web app must use the Redfish API to establish a session.
...
4. Handle BMC code update and BMC FRU replacement scenarios.  For
