...
2. The BMC's network interfaces (such as REST APIs, SSH, and IPMI)
   ...
   error message must be given.  Note the `/login` URI is deprecated.
   ...
   is expired.  It is not being enhanced by this design.
...
has an expired password, the web app will get an indication that the
password must be changed:
 ...
 - POST to `/redfish/v1/SessionService/Sessions` will indicate
   PasswordChangeRequired and establish a session.  The web app can
   use this session to change the password per the design above.
