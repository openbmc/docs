...
- Future enabled checks.
  - These are seen as useful checks for improving code quality but may require
    ...
    demonstrated sufficient enabling by proposing changes in multiple
    repositories to fix and enable those changes.
...
The [`clang-tidy` documentation][1] has a reference of all checks available but
be aware this may include checks not yet released as these documents are
generated from the clang git tree.
...
While the check seems promising in principle, the clang-tidy implementation
currently flags almost all non-constant indexes into `std::array` operations and
...
While we discourage magic numbers by our style, it is expected that this check
may be overly strict. Some experimentation is necessary to determine the
viability.
