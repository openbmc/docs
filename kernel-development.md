...
The OpenBMC project maintains a kernel tree for use by the project. The
tree's general development policy is that code must be upstream first. This is
strongly desirable but not a hard requirement, and exceptions may be made on a
...
choose to carry the patches in the OpenBMC tree while upstream development is
continues.
