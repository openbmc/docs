...
labels aka groups in the machine's led.yaml file.  The LED Manager then exposes
a DBus/REST interface for those groups to be acted upon.

### Defining the physical LED

Physical LED wiring is defined in the `leds` section of the machine's [device tree](https://github.com/openbmc/linux/tree/dev-4.10/arch/arm/boot/dts).
...
### Defining Groups
Groups of LEDs (which can simply contain a single LED) are defined in the machine's
