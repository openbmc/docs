...
Enabling field mode is intended to be a one-way operation.
...
Although field mode is meant to be a one-way operation it can be disabled by
running the following commands on the BMC and then rebooting the BMC:
...
On the host side, the factory reset will clear the rw volume for each host image
on the system, as well as the shared preserve host volume, pnor-prsv.
