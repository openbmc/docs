...
- Timeout while fetching image: verify `ipaddr`, `serverip`, subnet, and
  firewall rules.
- `Unknown command 'nfs'` in U-Boot: ensure NFS support is enabled in your
  U-Boot build.
- `Wrong Image Format` during `bootm`: confirm the kernel image and DTB match
  the target platform.
- Boot loops or early panic: verify kernel, DTB, and NFS rootfs are all from the
  same target build.
