 ...
 * `update_kernel_and_apps`: update kernel and initramfs. This setting is required if not using the full bmc image, which is only conatined in the `-all` tar-ball, and required to not be selected if using a whole bmc image tar-ball.
