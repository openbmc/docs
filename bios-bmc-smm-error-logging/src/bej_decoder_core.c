                            ...
                            bool canBeEmpty)
{
    ...
    while (!params->stackCallback->stackEmpty(params->stackDataPtr))
    {
        ...
        if (params->state.encodedStreamOffset == ending->streamEndOffset)
        {
            ...
            {
                RETURN_IF_IERROR(params->decodedCallback->callbackSetEnd(
                    params->callbacksDataPtr));
            }
            else if (ending->sectionType == BejSectionArray)
            {
                RETURN_IF_IERROR(params->decodedCallback->callbackArrayEnd(
                    params->callbacksDataPtr));
            }
        ...
        {
            RETURN_IF_IERROR(params->decodedCallback->callbackPropertyEnd(
                params->callbacksDataPtr));
...
static int bejHandleBejSet(struct BejHandleTypeFuncParam* params)
{
    ...
    {
        RETURN_IF_IERROR(
            params->decodedCallback->callbackSetEnd(params->callbacksDataPtr));
        goto bejEndSet;
    }
...
static int bejHandleBejInteger(struct BejHandleTypeFuncParam* params)
{
    ...
    RETURN_IF_IERROR(params->decodedCallback->callbackInteger(
        propName,
...
static int bejHandleBejEnum(struct BejHandleTypeFuncParam* params)
{
    ...
    RETURN_IF_IERROR(params->decodedCallback->callbackEnum(
        propName, enumValueName, params->callbacksDataPtr));
    params->state.encodedStreamOffset = params->sflv.valueEndOffset;
    return bejProcessEnding(params, /*canBeEmpty=*/false);
}
...
static int bejHandleBejString(struct BejHandleTypeFuncParam* params)
{
    ...
    RETURN_IF_IERROR(params->decodedCallback->callbackString(
        propName, (const char*)(params->sflv.value), params->callbacksDataPtr));
    params->state.encodedStreamOffset = params->sflv.valueEndOffset;
    return bejProcessEnding(params, /*canBeEmpty=*/false);
}
...
static int bejHandleBejReal(struct BejHandleTypeFuncParam* params)
{
    ...
    RETURN_IF_IERROR(params->decodedCallback->callbackReal(
        propName, &realValue, params->callbacksDataPtr));
    params->state.encodedStreamOffset = params->sflv.valueEndOffset;
    return bejProcessEnding(params, /*canBeEmpty=*/false);
}
...
static int bejHandleBejBoolean(struct BejHandleTypeFuncParam* params)
{
    ...
    RETURN_IF_IERROR(params->decodedCallback->callbackBool(
        propName, *(params->sflv.value) > 0, params->callbacksDataPtr));
    params->state.encodedStreamOffset = params->sflv.valueEndOffset;
    return bejProcessEnding(params, /*canBeEmpty=*/false);
}
