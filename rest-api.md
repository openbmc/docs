...
It is possible to subscribe to events, of interest, occuring on the BMC. The
...
pertianing to D-Bus InterfacesAdded and PropertiesChanged signals.
...
c) The client needs to send, over the websocket, a JSON dictionary, comprising
   of key-value pairs. This dict serves as the "events filter". The dict
   must be sent, but it can be empty, if no filtering is desired. The filters
   represented by each of the key-value pairs are ORed.
   ...
   be included in the event message going out of the BMC. This key is optional.

   One of the supported keys is "interfaces". The corresponding value is an
   array of D-Bus interfaces. The InterfacesAdded and PropertiesChanged D-Bus
   signal messages compirising of any of these interfaces will be included in
   th event message going out of the BMC. This key is optional.
