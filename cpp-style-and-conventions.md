...
tests on each repository so that regressions are quickly and prior to merge.
It also means having as much of this document enforced by tools as possible by, for example, astyle/clang-format and clang-tidy.
...
We do not currently utilize the Guideline Support Library provided by the CCG.  Any recommendation within the CCG of GSL conventions may be ignored at this
time.
