...
A reference .clang-tidy configuration file is provided to serve as a base
version for all OpenBMC repositories. This aligns with OpenBMC's recommended
standards and includes checks suggested by the
[clang-tidy documentation](https://clang.llvm.org/extra/clang-tidy/), covering
categories like bugprone, clang-analyzer, cppcoreguidelines, misc, performance,
and readability to enhance code quality and maintainability.

- Standardization: All repositories start with the same base configuration,
  ensuring consistent coding standards and checks.
- Customization:This reference file can be customized by enabling additional
  checks or full categories as needed. To maintain consistency across
  repositories, it is essential to follow the same template. Checks can be
  disabled by prefixing them with a -.
