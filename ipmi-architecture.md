...
```
   ...
   |            LAN - RMCP+      |                   |                  |
   | /--------------------------\|                   |                  |
   | |*Process the Session &    || <-All IPMI cmds-> |                  |
   | | SOL commands.            ||  except session   |                  |
   | |*Create Session Objs      ||  & SOL cmds       |                  |
   | \--------------------------/|                   |                  |
   \-----------------------------/                   \------------------/
                :                                             ^     ^
                :                                             |     |
...
The channel bridge, upon receiving a new IPMI command, will extract the
necessary fields from whatever transport format (RMCP+, IPMB, KCS, etc.) and
encode them in a D-Bus method call to send to the IPMI daemon, ipmid. The
daemon will take the message, and attempt to find an appropriate handler in
its handler tables. If a handler is found, it will attempt to extract the
required parameters for the handler and pass them along. The handler will
return a tuple of response parameters, which will get packed back into a
D-Bus response message and sent back to the calling channel's bridge. The
bridge will then re-package the response into its transport protocol and
send it off.
...
phosphor-ipmi-net manages session(RMCP+) and SOL commands directly and
responses to those commands. As these commands can also be requested through
host (KCS or BT) interface, phosphor-ipmi-host daemon must also need details
about session and SOL. In order to maintain sync between phosphor-ipmi-net and
phosphor-ipmi-host daemon, session and SOL are exposed in D-Bus, which
phosphor-ipmi-host can query and respond to commands issued through host
interface (KCS or BT).
...
D-Bus call mechanism that 'looks' like a synchronous call.
...
When returning partial byte types, they are also in the least-significant bit
first order. As each byte fills up, it it pushed onto the byte stream. For
examples of how the packing and unpacking is used, see the unit test cases in
test/message/pack.cpp and test/message/unpack.cpp.
